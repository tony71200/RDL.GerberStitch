...
namespace GerberViewer.Stitching.Stitching
{
    ...
    public sealed class StitchFromGlobalTransformsOptions
    {
        ...
        public BlankFallbackOverlapPolicy BlankFallbackOverlapPolicy { get; set; } =
            BlankFallbackOverlapPolicy.PreserveExistingOverlap;
        public StitchBlendMode EffectiveBlendMode
        {
            get {
                var mode = EnableBlending && !ForceGray8Output ? BlendMode : StitchBlendMode.NoBlend;
                if (BlankFallbackOverlapPolicy == BlankFallbackOverlapPolicy.WeightedBlend &&
                    mode == StitchBlendMode.NoBlend && !ForceGray8Output)
                    return StitchBlendMode.WeightedAverage;
                return mode;
            }
        }
    }
    ...
    public sealed class GlobalTransformStitcher
    {
        ...
        public string StitchFromGlobalTransforms(IList<CapturedImageInfo> images, IList<TileWorkflowState> poses,
                                                 StitchFromGlobalTransformsOptions options,
                                                 IProgress<StitchPreview> preview, CancellationToken cancellationToken)
        {
            if (images == null)
            ...
            options.ExecutionReport = new StitchingExecutionReport {
                ConfiguredEngine = options.StitchingEngine, EffectiveEngine = options.StitchingEngine,
            ...
            try
            {
                ...
                if (options.TiffMode == TiffMode.StandardTiff && selection.EstimatedBytes > 0xF0000000L)
                    throw new InvalidOperationException(
                        "Standard TIFF selected for an output estimated beyond the standard TIFF limit.");
                if (selection.RequiresBigTiff)
                    throw new NotSupportedException(
                        "BigTIFF output is required by size/configuration, but this writer does not claim BigTIFF " +
                        "support because it uses System.Drawing TIFF save.");
                EnsureDirectory(creatingPath);
                var effectiveEngine = options.StitchingEngine;
                // [Tony] [Change time: 2026-08-04] [Purpose: Stop silently ignoring the blending request on the HALCON
                // mosaic path.]
                ...
                {
                    const string blendingWarning =
                        "EnableBlending was requested but HOperatorSet.GenProjectiveMosaic has no blending " +
                        "parameter; " +
                ...
                // var requiresBlankOverlapPolicy = items.Any(x => x.Pose.Source == PoseSource.BlankSampleExpectedPose)
                // && options.BlankFallbackOverlapPolicy != BlankFallbackOverlapPolicy.Normal; if (effectiveEngine ==
                // StitchingEngine.HalconProjectiveMosaic && requiresBlankOverlapPolicy)
                ...
                if (effectiveEngine == StitchingEngine.HalconProjectiveMosaic)
                {
                    ...
                    options.ExecutionReport.CoordinateContract =
                        "Legacy all-to-root HALCON output frame (unsafe opt-in; not authoritative)";
                    new HalconProjectiveMosaicEngine().StitchLegacy(ToHalconInputs(items), creatingPath,
                                                                    selection.RequiresBigTiff, cancellationToken);
                ...
                else if (effectiveEngine == StitchingEngine.HalconProjectiveMosaicRebased)
                {
                    try
                    {
                        new HalconProjectiveMosaicEngine().StitchRebased(ToHalconInputs(items), bounds, width, height,
                                                                         creatingPath, selection.RequiresBigTiff,
                    ...
                    catch (Exception rebasedError)
                    {
                        ...
                        options.ExecutionReport.FallbackReason =
                            rebasedError.GetType().Name + ": " + rebasedError.Message;
                        options.ExecutionReport.CoordinateContract =
                            "CapturedImageLocalPixels -> ProcessedSampleGlobalPixels";
                        CleanupCreating(creatingPath);
                        using (var canvas =
                                   StitchToMat(items, bounds, width, height, options, preview, cancellationToken))
                        {
                            cancellationToken.ThrowIfCancellationRequested();
                ...
                else if (effectiveEngine == StitchingEngine.HalconWarpThenTileOffsetExperimental)
                {
                    try
                    {
                        if (options.EffectiveBlendMode != StitchBlendMode.NoBlend)
                            throw new NotSupportedException(
                                "HalconWarpThenTileOffsetExperimental supports overwrite/NoBlend only; Feather and " +
                                "WeightedAverage are not silently downgraded.");
                        new HalconWarpThenTileOffsetEngine().Stitch(
                            ToHalconInputs(items), bounds, width, height, creatingPath, selection.RequiresBigTiff,
                            options.BlankFallbackOverlapPolicy, options.ExecutionReport, cancellationToken);
                        ReopenAndValidate(creatingPath, width, height);
                    }
                    catch (OperationCanceledException)
                    {
                    ...
                    catch (Exception experimentalError)
                    {
                        options.ExecutionReport.EffectiveEngine = StitchingEngine.OpenCv;
                        options.ExecutionReport.FallbackReason =
                            experimentalError.GetType().Name + ": " + experimentalError.Message;
                        options.ExecutionReport.CoordinateContract =
                            "CapturedImageLocalPixels -> ProcessedSampleGlobalPixels";
                        CleanupCreating(creatingPath);
                        using (var canvas =
                ...
                else if (effectiveEngine == StitchingEngine.HalconThenOpenCvFallback)
                {
                    try
                    {
                        ...
                        options.ExecutionReport.CoordinateContract =
                            "Legacy all-to-root HALCON output frame (unsafe opt-in; not authoritative)";
                        // if (requiresBlankOverlapPolicy) throw new InvalidOperationException("The OpenCV occupied-mask
                        // path is required for the configured blank-overlap policy.");
                        new HalconProjectiveMosaicEngine().StitchLegacy(ToHalconInputs(items), creatingPath,
                                                                        selection.RequiresBigTiff, cancellationToken);
                        ReopenAndValidate(creatingPath, width, height);
                    }
                    ...
                    catch (Exception legacyError)
                    {
                        options.ExecutionReport.EffectiveEngine = StitchingEngine.OpenCv;
                        options.ExecutionReport.FallbackReason =
                            legacyError.GetType().Name + ": " + legacyError.Message;
                        options.ExecutionReport.CoordinateContract =
                            "CapturedImageLocalPixels -> ProcessedSampleGlobalPixels";
                        CleanupCreating(creatingPath);
                        using (var canvas =
                ...
                cancellationToken.ThrowIfCancellationRequested();
                // Treat the .creating-to-final transition as part of Save Image, matching the user-visible operation.
                var swPublish = System.Diagnostics.Stopwatch.StartNew();
                Publish(creatingPath, output);
                swPublish.Stop();
                if (options.ExecutionReport != null)
                    options.ExecutionReport.SaveElapsedMilliseconds += swPublish.ElapsedMilliseconds;
                return output;
            }
            catch (OperationCanceledException)
            {
            ...
            {
                // [Tony] [Change time: 2026-07-27] [Purpose: Cleanup must never replace a successful return or the
                // original stitch exception.]
        ...
        {
            return items
                .Select(x => new HalconMosaicInput { Image = x.Image, GlobalPose = x.Pose.GlobalPose,
        ...
        private Mat StitchToMat(IList<StitchItem> items, RectangleF bounds, int width, int height,
                                StitchFromGlobalTransformsOptions options, IProgress<StitchPreview> preview,
                                CancellationToken cancellationToken)
        {
            ...
            Mat canvas8 =
                blendMode == StitchBlendMode.NoBlend ? new Mat(height, width, canvasType, Scalar.All(0)) : null;
            Mat accum =
                blendMode == StitchBlendMode.NoBlend ? null : new Mat(height, width, MatType.CV_32FC3, Scalar.All(0));
            Mat weights =
                blendMode == StitchBlendMode.NoBlend ? null : new Mat(height, width, MatType.CV_32FC1, Scalar.All(0));
            ...
            try
            {
                for (int idx = 0; idx < items.Count; idx++)
                {
                    cancellationToken.ThrowIfCancellationRequested();
                    var item = items[idx];
                    var imagePath = item.Image.FilePath;
                    var forceGray8Output = options.ForceGray8Output;

                    using (var image = item.IsBlackPlaceholder
                        ? CreateBlackTile(item.Image, forceGray8Output)
                        : LoadForStitch(imagePath, forceGray8Output))
                    using (var mask = new Mat(
                        image.Rows,
                        image.Cols,
                        MatType.CV_8UC1,
                        Scalar.All(255)))
                    using (var warp = ToCanvasWarp(
                        item.Pose.GlobalPose,
                        bounds))
                    using (var warped = new Mat())
                    using (var warpedMask = new Mat())
                    using (var effectiveMask = new Mat())
                    {
                        Cv2.WarpAffine(image, warped, warp, new OpenCvSharp.Size(width, height),
                                       InterpolationFlags.Linear, BorderTypes.Constant, Scalar.All(0));
                        Cv2.WarpAffine(mask, warpedMask, warp, new OpenCvSharp.Size(width, height),
                                       InterpolationFlags.Nearest, BorderTypes.Constant, Scalar.All(0));
                        warpedMask.CopyTo(effectiveMask);
                        var preserve =
                            items[idx].Pose.Source == PoseSource.BlankSampleExpectedPose &&
                            options.BlankFallbackOverlapPolicy == BlankFallbackOverlapPolicy.PreserveExistingOverlap;
                        if (preserve)
                        {
                            using (var unoccupied = new Mat())
                            {
                                Cv2.BitwiseNot(occupiedMask, unoccupied);
                                Cv2.BitwiseAnd(effectiveMask, unoccupied, effectiveMask);
                            }
                        }
                        if (blendMode == StitchBlendMode.NoBlend)
                            warped.CopyTo(canvas8, effectiveMask);
                        else
                            AccumulateBlend(warped, effectiveMask, accum, weights, blendMode);
                        Cv2.BitwiseOr(occupiedMask, warpedMask, occupiedMask);
                    }
                    if (preview != null && options.PreviewUpdateInterval > 0 &&
                        ((idx + 1) % options.PreviewUpdateInterval == 0 || idx == items.Count - 1))
                    {
                        using (var current = blendMode == StitchBlendMode.NoBlend ? canvas8.Clone()
                                                                                  : ResolveBlend(accum, weights))
                            preview.Report(
                                new StitchPreview { Preview = MakePreview(current, options.MaxPreviewMegapixels),
                                                    PlacedCount = idx + 1, TotalCount = items.Count });
                    }
                }
                if (blendMode == StitchBlendMode.NoBlend)
                {
            ...
            finally
            {
                ...
                if (occupiedMask != null)
                    occupiedMask.Dispose();
            }
        }

        private static void AccumulateBlend(Mat warped, Mat warpedMask, Mat accum, Mat weights,
                                            StitchBlendMode blendMode)
        {
            using (var image32 = new Mat()) using (var weight = BuildWeight(warpedMask, blendMode)) using (
                var weight3 = To3Channels(weight)) using (var weightedImage = new Mat())
            {
        ...
        private static Mat BuildWeight(Mat mask, StitchBlendMode blendMode)
        {
            ...
            if (blendMode == StitchBlendMode.Feather)
            {
                ...
                mask.ConvertTo(weight, MatType.CV_32FC1, 1.0 / 255.0);
            return weight;
        }
        ...
        {
            using (var safeWeights = new Mat()) using (var weight3 = new Mat()) using (var output32 = new Mat())
            {
        ...
        {
            return image.Width > 0 && image.Height > 0 ? new System.Drawing.Size(image.Width, image.Height)
        ...
        private static Mat ToCanvasWarp(double[,] transform, RectangleF bounds)
        {
            ...
            warp.Set<double>(0, 0, transform[0, 0]);
            warp.Set<double>(0, 1, transform[0, 1]);
            warp.Set<double>(0, 2, transform[0, 2] - bounds.Left);
            warp.Set<double>(1, 0, transform[1, 0]);
            warp.Set<double>(1, 1, transform[1, 1]);
            warp.Set<double>(1, 2, transform[1, 2] - bounds.Top);
            return warp;
        }
        ...
        {
            var duplicateImages =
                images.GroupBy(i => i.OrderIndex).Where(g => g.Count() > 1).Select(g => g.Key).ToArray();
            if (duplicateImages.Length != 0)
                throw new InvalidOperationException(
                    "Duplicate captured image OrderIndex: " + string.Join(",", duplicateImages) + ".");
            var duplicatePoses =
                poses.GroupBy(p => p.OrderIndex).Where(g => g.Count() > 1).Select(g => g.Key).ToArray();
            if (duplicatePoses.Length != 0)
                throw new InvalidOperationException(
                    "Duplicate stitching pose OrderIndex: " + string.Join(",", duplicatePoses) + ".");
            ...
            foreach (var pose in poses.Where(p => p.IsStitchable || (p.Source == PoseSource.Failed && p.HasValidPose))
                         .OrderBy(p => p.Source == PoseSource.Failed ? 1 : 0)
                         .ThenBy(p => p.OrderIndex))
            {
                CapturedImageInfo image;
                if (imageByOrder.TryGetValue(pose.OrderIndex, out image))
                    result.Add(new StitchItem(image, pose));
            }
            return result;
        }
        ...
        public static RectangleF CalculateBounds(IList<Tuple<CapturedImageInfo, double[,]>> items)
        {
            if (items == null || items.Count == 0)
                throw new ArgumentException("At least one transformed image is required.", "items");
            double left = double.PositiveInfinity, top = double.PositiveInfinity, right = double.NegativeInfinity,
            ...
            return RectangleF.FromLTRB((float)Math.Floor(left), (float)Math.Floor(top), (float)Math.Ceiling(right),
                                       (float)Math.Ceiling(bottom));
        }
        ...
        private static IEnumerable<PointF> TransformCorners(double[,] h, int width, int height)
        {
            ...
            foreach (var p in pts)
                yield return new PointF((float)(h[0, 0] * p.X + h[0, 1] * p.Y + h[0, 2]),
                                        (float)(h[1, 0] * p.X + h[1, 1] * p.Y + h[1, 2]));
        }
        ...
        public static TiffOutputSelection SelectTiffOutput(TiffMode mode, int width, int height, int bytesPerPixel)
        {
            ...
            if (mode == TiffMode.BigTiff)
                return new TiffOutputSelection(true, bytes, "Configured BigTIFF mode.");
            if (mode == TiffMode.StandardTiff)
                return new TiffOutputSelection(false, bytes, "Configured StandardTIFF mode.");
            return new TiffOutputSelection(bytes > 0xF0000000L || width > 65500 || height > 65500, bytes,
                                           "Auto selection from dimensions and byte count.");
        }
        ...
        // Measure TIFF writing time separately for the Execute Time tab.
        ...
        {
            var scale =
                Math.Min(1.0, Math.Sqrt((maxPreviewMegapixels * 1000000.0) / Math.Max(1.0, src.Cols * src.Rows)));
            using (var previewMat = new Mat())
            {
                if (scale >= .999)
                    src.CopyTo(previewMat);
                else
                    Cv2.Resize(src, previewMat,
                               new OpenCvSharp.Size(Math.Max(1, (int)(src.Cols * scale)),
                                                    Math.Max(1, (int)(src.Rows * scale))),
        ...
        private static void Publish(string creatingPath, string output)
        {
            ...
            if (File.Exists(output))
                throw new IOException(
                    "Publish conflict: the stitched output already exists and will not be replaced: " + output);
        ...
        private static void TryCleanupCreating(string creatingPath)
        {
            try
            {
                ...
                if (IsOwnedCreatingDirectory(directory) && Directory.Exists(directory) &&
        ...
        private static bool IsOwnedCreatingDirectory(string directory)
        {
            return !string.IsNullOrWhiteSpace(directory) &&
                   string.Equals(
                       ...
                       ".creating", StringComparison.OrdinalIgnoreCase);
        }
        ...
        {
            using (var image = OpenCvInput.DecodeBgr8Copy(path)) return new System.Drawing.Size(image.Cols, image.Rows);
        }
        ...
        private sealed class StitchItem
        {
            ...
            public bool IsBlackPlaceholder
            {
                get {
                    return Pose.Source == PoseSource.Failed;
                }
            }
        }
    }
    ...
    {
        public TiffOutputSelection(bool requiresBigTiff, long estimatedBytes, string reason)
        {
            RequiresBigTiff = requiresBigTiff;
    ...
    // [Tony] [Change time: 2026-08-01] [Purpose: Prevent the legacy all-to-root HALCON implementation from publishing
    // an unverified coordinate frame.]
    public static class LegacyHalconSafetyGate
    {
        public const string BlockReason =
            "Legacy HALCON ProjectiveMosaic is blocked because its all-to-root output coordinate contract is not " +
            "verified. Enable the unsafe opt-in only for controlled comparison runs.";
