...
{
    // [Tony] [Change time: 2026-07-27] [Purpose: Centralize application-owned run publication and failure cleanup
    // safety.]
    public static class RunOutputLifecycle
    {
        public static void Publish(string finalRunDirectory, string creatingDirectory)
        {
            ...
            if (!Directory.Exists(creatingDirectory))
                throw new DirectoryNotFoundException("Creating directory missing before publish: " + creatingDirectory);
        ...
        public static void Cleanup(string finalRunDirectory, string creatingDirectory)
        {
            try
            {
                ValidateOwnedCreatingDirectory(finalRunDirectory, creatingDirectory);
                if (Directory.Exists(creatingDirectory))
                    Directory.Delete(creatingDirectory, true);
                if (Directory.Exists(finalRunDirectory) &&
                    !Directory.EnumerateFileSystemEntries(finalRunDirectory).Any())
                    Directory.Delete(finalRunDirectory, false);
            }
            catch
            {
        ...
        {
            if (string.IsNullOrWhiteSpace(finalRunDirectory))
                throw new ArgumentException("Final run directory is required.", "finalRunDirectory");
            var expected = Path.GetFullPath(Path.Combine(finalRunDirectory, ".creating"))
                               .TrimEnd(Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar);
            var actual = Path.GetFullPath(creatingDirectory ?? string.Empty)
                             .TrimEnd(Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar);
            if (!string.Equals(expected, actual, StringComparison.OrdinalIgnoreCase))
                throw new InvalidOperationException(
                    "Cleanup/publish is restricted to the current run's application-owned .creating directory.");
        }
    }
}
