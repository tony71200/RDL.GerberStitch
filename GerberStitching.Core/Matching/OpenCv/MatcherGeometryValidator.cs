    ...
    public sealed class MatcherGeometryValidator
    {
        public MatchResult ValidateRequest(MatchRequest request, string matcherName)
        {
            ...
            if (!IsRoiValid(request.ReferenceImage, request.ReferenceRoi)) 
                return MatchResult.Failed(matcherName, MatchFailureReason.InvalidRoi, "ReferenceRoi is outside ReferenceImage bounds.");
            if (!IsRoiValid(request.MovingImage, request.MovingRoi)) 
                return MatchResult.Failed(matcherName, MatchFailureReason.InvalidRoi, "MovingRoi is outside MovingImage bounds.");
            return null;
        }
        ...
        public MatchResult ValidatePreparedPair(Mat reference, Mat moving, MatcherOptions options, string matcherName)
        {
            if (reference == null || moving == null || reference.Empty() || moving.Empty()) 
                return MatchResult.Failed(matcherName, MatchFailureReason.InvalidInput, "Prepared images are required.");
            if (reference.Rows != moving.Rows || reference.Cols != moving.Cols) 
                return MatchResult.Failed(matcherName, MatchFailureReason.SizeMismatch, "ReferenceImage and MovingImage ROI must have the same size for phase correlation.");
            ...
            if (overlap < options.MinOverlapRatio) 
                return MatchResult.Failed(matcherName, MatchFailureReason.SizeMismatch, "Prepared image overlap ratio is below threshold.");
            if (CalculateStdDev(reference) < options.MinTextureStdDev || CalculateStdDev(moving) < options.MinTextureStdDev) 
                return MatchResult.Failed(matcherName, MatchFailureReason.LowTexture, "ReferenceImage or MovingImage texture is below MinTextureStdDev.");
            return null;
        }
        ...
        public MatchResult ValidateTranslation(double translationX, double translationY, double response, MatcherOptions options, string matcherName)
        {
            ...
            if (Math.Abs(response) < options.PhaseMinResponse) return MatchResult.Failed(matcherName, MatchFailureReason.ResponseBelowThreshold, "Phase correlation response is below PhaseMinResponse.");
            return null;
        }
