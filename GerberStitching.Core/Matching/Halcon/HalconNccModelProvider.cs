...
{
    // [Codex] [Change time: 2026-07-26] [Purpose: Give one IDisposable owner responsibility for every cached HALCON NCC model ID.]
    public sealed class HalconNccModelProvider : IDisposable
    {
        ...
        public HalconNccModelProvider(IImageInteropService interop) { _inputAdapter = new HalconMatchInputAdapter(interop); }
        public int CachedModelCount { get { lock (_sync) return _cache.Count; } }
        ...
        private Entry CreateEntry(MatchRequest request, MatcherOptions options)
        {
            ...
            try
            {
                ...
                var hasVersionedDiskModel = request.NccModelSchemaVersion == 1 && request.ReferenceNccModelRoi.HasValue &&
                    request.NccReferenceOriginRow.HasValue && request.NccReferenceOriginColumn.HasValue &&
                    !string.IsNullOrWhiteSpace(request.ReferenceNccModelPath) && File.Exists(request.ReferenceNccModelPath);
                double localRow;
                double localColumn;
                ...
                else
                {
                    ...
                    HOperatorSet.CreateNccModel(modelImage, options.NccNumLevels, options.NccAngleStartRad, options.NccAngleExtentRad, options.NccAngleStepRad, options.NccMetric, out modelId);
                }
                ...
                {
                    DomainCenterRowLocal = localRow, DomainCenterColumnLocal = localColumn,
                    ReferenceOriginRow = referenceOriginRow, ReferenceOriginColumn = referenceOriginColumn,
                    ConfiguredOffsetRow = offsetRow, ConfiguredOffsetColumn = offsetColumn,
                    VerifiedOffsetRow = verifiedRow[0].D, VerifiedOffsetColumn = verifiedColumn[0].D,
                    Convention = "Model origin represents full reference tile top-left"
                };
                var entry = new Entry(modelId, roi, origin, 1, hasVersionedDiskModel ? "VersionedDiskModel" : "RegeneratedOrRuntimeModel");
                modelId = null;
                return entry;
            }
            finally
        ...
        {
            if (request.ReferenceNccModelRoi.HasValue) return ValidateRoi(request.ReferenceNccModelRoi.Value, request.ReferenceImage);
            using (var gray = new Mat())
            using (var mask = new Mat())
            using (var filteredMask = new Mat())
            using (var nonZeroPoints = new Mat())
            {
                if (request.ReferenceImage.Channels() == 1) request.ReferenceImage.CopyTo(gray); else Cv2.CvtColor(request.ReferenceImage, gray, ColorConversionCodes.BGR2GRAY);
                // [Codex] [Change time: 2026-07-27] [Purpose: Use the OpenCvSharp API shape provided by the repository's pinned package version.]
                ...
                Rect roi;
                if (nonZeroPoints.Empty()) throw new InvalidOperationException("Exact-zero reference cannot create an NCC content model.");
                ...
                if (roi.Width < options.NccModelRoiMinWidth || roi.Height < options.NccModelRoiMinHeight) throw new InvalidOperationException("NCC content ROI is below configured minimum size.");
                if (roi.Width == gray.Cols && roi.Height == gray.Rows) throw new InvalidOperationException("A full-size NCC model has no translation search margin in a same-size search image.");
                Scalar mean, stddev;
        ...
        {
            if (roi.X < 0 || roi.Y < 0 || roi.Width <= 0 || roi.Height <= 0 || roi.Right > image.Cols || roi.Bottom > image.Rows)
        ...
        private static string BuildCacheKey(MatchRequest request, MatcherOptions options)
        {
            ...
            var tile = !string.IsNullOrWhiteSpace(request.SampleTileId) ? request.SampleTileId : (request.OrderIndex.HasValue ? request.OrderIndex.Value.ToString(CultureInfo.InvariantCulture) : "__unspecified_tile__");
            return string.Join("|", new[] {
                "Tile=" + tile, "Model=" + (path ?? "__none__"), "Stamp=" + ModelStamp(path),
                ...
                "Roi=" + RoiKey(request.ReferenceNccModelRoi), "Schema=" + request.NccModelSchemaVersion.ToString(CultureInfo.InvariantCulture),
                "Origin=" + NullableDouble(request.NccReferenceOriginRow) + "," + NullableDouble(request.NccReferenceOriginColumn),
                ...
                "Metric=" + options.NccMetric, "Levels=" + options.NccNumLevels.ToString(CultureInfo.InvariantCulture)
            });
        }
        private static string RoiKey(Rect? roi) { return roi.HasValue ? roi.Value.X + "," + roi.Value.Y + "," + roi.Value.Width + "," + roi.Value.Height : "auto"; }
        private static string NullableDouble(double? value) { return value.HasValue ? value.Value.ToString("R", CultureInfo.InvariantCulture) : "auto"; }
        private static void DisposeTuple(HTuple tuple) { if (tuple != null) tuple.Dispose(); }
        private static string NormalizePath(string path) { if (string.IsNullOrWhiteSpace(path)) return null; try { return Path.GetFullPath(path); } catch { return path; } }
        private static string ModelStamp(string path) { if (string.IsNullOrWhiteSpace(path) || !File.Exists(path)) return "__not_available__"; var i = new FileInfo(path); return i.LastWriteTimeUtc.Ticks + ":" + i.Length; }
        private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName); }
        public void Dispose() { lock (_sync) { if (_disposed) return; foreach (var e in _cache.Values) e.Dispose(); _cache.Clear(); _disposed = true; } }

        private sealed class Entry : IDisposable
        {
            public Entry(HTuple modelId, Rect roi, HalconModelOrigin origin, int schemaVersion, string source) { ModelId = modelId ?? throw new ArgumentNullException("modelId"); ModelRoi = roi; Origin = origin; SchemaVersion = schemaVersion; Source = source; UseCount = 1; }
            ...
            public HalconNccModel ToModel(string key, bool hit) { return new HalconNccModel(ModelId, key, hit, ModelRoi, Origin, SchemaVersion, Source); }
            public void Dispose() { if (ModelId == null) return; HOperatorSet.ClearNccModel(ModelId); ModelId.Dispose(); ModelId = null; }
        }
    }

    public sealed class HalconNccModel
    {
        internal HalconNccModel(HTuple modelId, string cacheKey, bool cacheHit, Rect roi, HalconModelOrigin origin, int schemaVersion, string source) { ModelId = modelId; CacheKey = cacheKey; CacheHit = cacheHit; ModelRoi = roi; Origin = origin; SchemaVersion = schemaVersion; Source = source; }
        public HTuple ModelId { get; private set; }
