...
namespace GerberViewer.Stitching.Matching.Halcon
{
    public sealed class HalconNccMatcher : IMatcher
    {
        ...
        public MatchResult Match(MatchRequest request, CancellationToken cancellationToken)
        {
            ...
            if (cancellationToken.IsCancellationRequested)
                return Complete(request,
                                MatchResult.Failed(MatcherName, MatchFailureReason.Cancelled,
                                                   "HALCON NCC was cancelled before start."),
                                sw);
            ...
            try
            {
                using (var movingHObject = _inputAdapter.ToMono8HObjectCopy(request.MovingImage))
                {
                    ...
                    HOperatorSet.FindNccModel(movingHObject, model.ModelId, options.NccAngleStartRad,
                                              options.NccAngleExtentRad, options.NccMinScore, options.NccMaxMatches,
                                              options.NccMaxOverlap, options.NccSubPixel, options.NccNumLevels, out row,
                                              out column, out angle, out score);
                    cancellationToken.ThrowIfCancellationRequested();
                    if (score == null || score.Length <= 0)
                        return Complete(
                            request,
                            CreateCenterFallback(request, options,
                                                 "HALCON find_ncc_model returned no match above NccMinScore."),
                            sw);
                    ...
                    if (bestScore < options.NccMinScore)
                        return Complete(
                            request, CreateCenterFallback(request, options, "HALCON NCC score is below NccMinScore."),
                            sw);

                    var movingToReference = HalconModelPoseConverter.ToMovingToReference(
                        bestColumn, bestRow, bestAngle, model.Origin.ReferenceOriginColumn,
                        model.Origin.ReferenceOriginRow);
                    var geometryFailure =
                        ValidateNccGeometry(movingToReference, request.ReferenceImage, request.MovingImage, options);
                    if (geometryFailure != null)
                        return Complete(request, CreateCenterFallback(request, options, geometryFailure.FailureMessage),
                                        sw);
                    var result =
                        new MatchResult { Success = true,
                                          MatcherName = MatcherName,
                                          MovingToReferenceTransform = movingToReference,
                                          TranslationX = movingToReference[0, 2],
                                          TranslationY = movingToReference[1, 2],
                                          RotationDeg = Math.Atan2(movingToReference[1, 0], movingToReference[0, 0]) *
                                                        180.0 / Math.PI,
                                          Scale = 1d,
                                          RawScore = bestScore,
                    ...
                    result.Diagnostics["ModelSchemaVersion"] =
                        model.SchemaVersion.ToString(CultureInfo.InvariantCulture);
                    result.Diagnostics["ModelSource"] = model.Source;
                    result.Diagnostics["ModelRoi"] = model.ModelRoi.X + "," + model.ModelRoi.Y + "," +
                                                     model.ModelRoi.Width + "," + model.ModelRoi.Height;
                    result.Diagnostics["DomainCenterLocal"] =
                        model.Origin.DomainCenterRowLocal.ToString("R", CultureInfo.InvariantCulture) + "," +
                        model.Origin.DomainCenterColumnLocal.ToString("R", CultureInfo.InvariantCulture);
                    result.Diagnostics["ReferenceOrigin"] =
                        model.Origin.ReferenceOriginRow.ToString("R", CultureInfo.InvariantCulture) + "," +
                        model.Origin.ReferenceOriginColumn.ToString("R", CultureInfo.InvariantCulture);
                    result.Diagnostics["ConfiguredOriginOffset"] =
                        model.Origin.ConfiguredOffsetRow.ToString("R", CultureInfo.InvariantCulture) + "," +
                        model.Origin.ConfiguredOffsetColumn.ToString("R", CultureInfo.InvariantCulture);
                    result.Diagnostics["VerifiedOriginOffset"] =
                        model.Origin.VerifiedOffsetRow.ToString("R", CultureInfo.InvariantCulture) + "," +
                        model.Origin.VerifiedOffsetColumn.ToString("R", CultureInfo.InvariantCulture);
                    ...
                    result.Diagnostics["ReferenceToMovingTransform"] =
                        HalconModelPoseConverter.Format(movingToReference.Invert());
                    result.Diagnostics["MovingToReferenceTransform"] =
                        HalconModelPoseConverter.Format(movingToReference);
            ...
            {
                return Complete(
                    request, MatchResult.Failed(MatcherName, MatchFailureReason.Cancelled, "HALCON NCC was cancelled."),
                    sw);
            }
            catch (Exception ex)
            {
                return Complete(
                    request, CreateCenterFallback(request, request == null ? null : request.Options, ex.Message), sw);
            }
            finally
            {
                DisposeTuple(row);
        ...
        {
            get {
                return _modelProvider.CachedModelCount;
            }
        }
        ...
        {
            // [Tony] [Change time: 2026-07-26] [Purpose: A missing NCC match is failure; an expected/center pose must
            // never become measured success.]
            var result = MatchResult.Failed(MatcherName, MatchFailureReason.ResponseBelowThreshold, reason);
            result.OverlapRatio =
                request == null
                    ? 0d
                    : MatcherGeometryValidator.CalculateSameSizeOverlap(request.ReferenceImage, request.MovingImage);
            result.Diagnostics["Fallback"] = "false";
            result.Diagnostics["FailureReason"] = reason ?? string.Empty;
            return result;
        }

        private static Mat WarpMovingToReference(Mat movingImage, Transform2D movingToReference,
                                                 OpenCvSharp.Size referenceSize)
        {
            ...
            try
            {
                ...
                var aligned = new Mat();
                Cv2.WarpAffine(movingImage, aligned, warp, referenceSize, InterpolationFlags.Linear,
        ...
        private MatchResult ValidateNccGeometry(Transform2D movingToReference, Mat referenceImage, Mat movingImage,
                                                MatcherOptions options)
        {
            if (movingToReference == null)
                return MatchResult.Failed(MatcherName, MatchFailureReason.NonFiniteTransform,
                                          "HALCON NCC did not produce a transform.");
            var translationFailure = _validator.ValidateTranslation(movingToReference[0, 2], movingToReference[1, 2],
                                                                    1.0, options, MatcherName);
            ...
            if (Math.Abs(rotationDeg) > options.MaxAbsRotationDeg)
                return MatchResult.Failed(MatcherName, MatchFailureReason.GeometryRejected,
                                          "HALCON NCC rotation exceeds MaxAbsRotationDeg.");
            var scaleX = Math.Sqrt(movingToReference[0, 0] * movingToReference[0, 0] +
                                   movingToReference[1, 0] * movingToReference[1, 0]);
            var scaleY = Math.Sqrt(movingToReference[0, 1] * movingToReference[0, 1] +
                                   movingToReference[1, 1] * movingToReference[1, 1]);
            if (scaleX < options.MinScale || scaleX > options.MaxScale || scaleY < options.MinScale ||
                scaleY > options.MaxScale)
                return MatchResult.Failed(MatcherName, MatchFailureReason.GeometryRejected,
                                          "HALCON NCC scale is outside configured bounds.");
            ...
            if (overlap < options.MinOverlapRatio)
                return MatchResult.Failed(MatcherName, MatchFailureReason.GeometryRejected,
                                          "HALCON NCC image overlap is below MinOverlapRatio.");
            return null;
        }
