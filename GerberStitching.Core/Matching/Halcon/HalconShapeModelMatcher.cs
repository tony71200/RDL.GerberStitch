...
{
    // [Tony] [Change time: 2026-07-27] [Purpose: Match AlignByShapeModel search behavior while preserving canonical
    // Moving-to-Reference output.]
    public sealed class HalconShapeModelMatcher : IMatcher
    {
        ...
        {
            get {
                return "HalconShapeModelMatcher";
            }
        }
        ...
        {
            get {
                return _provider.CachedModelCount;
            }
        }
        ...
        public MatchResult Match(MatchRequest request, CancellationToken token)
        {
            ...
            if (token.IsCancellationRequested)
                return Complete(request, Fail(MatchFailureReason.Cancelled, "HALCON Shape was cancelled before start."),
                                stopwatch);

            var invalid = _validator.ValidateRequest(request, MatcherName);
            if (invalid != null)
                return Complete(request, invalid, stopwatch);

            var invalidShapeRoi = ValidateOptionalShapeRoi(request);
            if (invalidShapeRoi != null)
                return Complete(request, invalidShapeRoi, stopwatch);
            ...
            try
            {
                using (var moving = _adapter.ToMono8HObjectCopy(request.MovingImage))
                {
                    ...
                    HOperatorSet.FindShapeModel(moving, model.ModelId, options.AngleStartRad, options.AngleExtentRad,
                                                options.MinScore, options.NumMatches, options.MaxOverlap,
                                                options.SubPixel, options.SearchNumLevels, options.Greediness, out row,
                    ...
                    if (score == null || score.Length == 0)
                        return Complete(
                            request,
                            Fail(MatchFailureReason.ResponseBelowThreshold, "find_shape_model returned no result."),
                            stopwatch);

                    var rawScore = score[0].D;
                    if (rawScore < options.MinScore)
                        return Complete(
                            request, Fail(MatchFailureReason.ResponseBelowThreshold, "Shape score is below MinScore."),
                            stopwatch);

                    var transform = HalconModelPoseConverter.ToMovingToReference(column[0].D, row[0].D, angle[0].D, 1.0,
                                                                                 model.Origin.ReferenceOriginColumn,
                                                                                 model.Origin.ReferenceOriginRow);

                    var translationFailure = _validator.ValidateTranslation(transform[0, 2], transform[1, 2], 1.0,
                                                                            request.Options, MatcherName);
                    if (translationFailure != null)
                        return Complete(request, translationFailure, stopwatch);
                    ...
                    if (Math.Abs(rotationDeg) > request.Options.MaxAbsRotationDeg)
                        return Complete(
                            request,
                            Fail(MatchFailureReason.GeometryRejected, "Shape rotation exceeds MaxAbsRotationDeg."),
                            stopwatch);
                    ...
                    var result = new MatchResult { Success = true,
                                                   ...
                                                   OverlapRatio = MatcherGeometryValidator.CalculateSameSizeOverlap(
                                                       request.ReferenceImage, request.MovingImage),
                    ...
                    result.Diagnostics["ReferenceOrigin"] =
                        model.Origin.ReferenceOriginRow + "," + model.Origin.ReferenceOriginColumn;
                    ...
                    result.Diagnostics["DetectedDxLikeHDevelop"] =
                        (column[0].D - model.Origin.ReferenceOriginColumn).ToString("R", CultureInfo.InvariantCulture);
                    result.Diagnostics["DetectedDyLikeHDevelop"] =
                        (row[0].D - model.Origin.ReferenceOriginRow).ToString("R", CultureInfo.InvariantCulture);
            ...
            catch (HalconImageInteropException ex)
            {
                return Complete(
                    request,
                    Fail(MatchFailureReason.RuntimeFailure,
                         ex.Message + " Shape matching was rejected; no score or transform was published."),
                    stopwatch);
            }
        ...
        {
            if (!request.ReferenceShapeModelRoi.HasValue)
                return null;
