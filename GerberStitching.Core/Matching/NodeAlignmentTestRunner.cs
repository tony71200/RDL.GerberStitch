...
namespace GerberViewer.Stitching.Matching
{
    ...
    public sealed class NodeAlignmentTestRunner
    {
        ...
        // [Codex] [Change time: 2026-07-26] [Purpose: Isolate diagnostic matching, transform direction, mask generation and preview ownership from WinForms.]
        public NodeAlignmentTestResult Run(NodeAlignmentTestContext context, NodeAlignmentTestMode testMode, CancellationToken cancellationToken)
        {
            Validate(context);
            cancellationToken.ThrowIfCancellationRequested();
            using (var reference = _input.DecodeMono8Copy(context.SampleImagePath))
            using (var moving = _input.DecodeMono8Copy(context.CapturedImagePath))
            {
                ...
                var request = new MatchRequest
                {
                    ...
                    Purpose = testMode == NodeAlignmentTestMode.ProductionPipeline ? MatchPurpose.CapturedToSample : MatchPurpose.ManualPreview,
                ...
                stopwatch.Stop();
                if (match == null) match = MatchResult.Failed(testMode.ToString(), MatchFailureReason.RuntimeFailure, "Matcher returned no result.");
                if (match.ProcessingTime == TimeSpan.Zero) match.ProcessingTime = stopwatch.Elapsed;
                cancellationToken.ThrowIfCancellationRequested();
                ...
                using (var transformed = WarpMovingToReference(moving, reference.Size(), match.MovingToReferenceTransform))
                using (var contentMask = GenerateContentMask(reference))
                using (var overlay = CreateRedMaskOverlay(transformed, contentMask))
        ...
        private MatchResult RunMatch(MatchRequest request, NodeAlignmentTestContext context, NodeAlignmentTestMode testMode, CancellationToken token)
        {
            if (testMode == NodeAlignmentTestMode.ProductionPipeline)
            {
                ...
                var result = pipeline.MatchDirect(
                    request,
                    context.ProductionCoarseMatcher,
                    context.ProductionRefinementMatcher,
                    context.AllowCoarseOnlyAcceptance,
                    context.AllowRefinementFromExpectedWhenCoarseFails,
                    token);
                return result.SelectedCandidate ?? MatchResult.Failed("ProductionPipeline", MatchFailureReason.ResponseBelowThreshold, result.FailureReason);
            }

            var matcherKind = ToMatcherKind(testMode);
            using (var matcher = _factory.Create(matcherKind, _resultSink))
                return matcher.Match(request, token);
        }
        ...
        private static MatcherKind ToMatcherKind(NodeAlignmentTestMode testMode)
        {
            switch (testMode)
            {
                case NodeAlignmentTestMode.HalconNcc: return MatcherKind.HalconNcc;
                case NodeAlignmentTestMode.HalconShapeModel: return MatcherKind.HalconShapeModel;
                case NodeAlignmentTestMode.PyramidEcc: return MatcherKind.PyramidEcc;
                default: throw new ArgumentOutOfRangeException("testMode", testMode, "Unsupported single matcher test mode.");
            }
        }
        ...
        private static Mat WarpMovingToReference(Mat moving, OpenCvSharp.Size referenceSize, GerberViewer.Stitching.Transforms.Transform2D transform)
        {
            var matrix = transform.ToArray();
            using (var affine = new Mat(2, 3, MatType.CV_64FC1))
            {
                for (var row = 0; row < 2; row++) for (var column = 0; column < 3; column++) affine.Set<double>(row, column, matrix[row, column]);
                var transformed = new Mat();
                Cv2.WarpAffine(moving, transformed, affine, referenceSize, InterpolationFlags.Linear, BorderTypes.Constant, Scalar.Black);
                return transformed;
            }
        }
        ...
        {
            using (var baseColor = ToBgr(transformed))
            using (var red = new Mat(baseColor.Size(), baseColor.Type(), new Scalar(0, 0, 255)))
        ...
        private static Mat ResizeBounded(Mat source)
        {
            ...
            if (scale >= 1d) return source.Clone();
            var resized = new Mat();
            Cv2.Resize(source, resized, new OpenCvSharp.Size(Math.Max(1, (int)Math.Round(source.Width * scale)), Math.Max(1, (int)Math.Round(source.Height * scale))), 0, 0, InterpolationFlags.Area);
            return resized;
        }
    }
}
