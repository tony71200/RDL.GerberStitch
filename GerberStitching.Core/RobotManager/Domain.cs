...
namespace GerberViewer.Stitching.RobotManager
{
    ...
    public static class StartOrderResolver
    {
        public static StartOrderResolution Resolve(StartOrder order)
        {
            ...
            case StartOrder.TopLeftRight:
                return new StartOrderResolution { StartCorner = StartCorner.TopLeft,
            ...
            case StartOrder.BottomRightLeft:
                return new StartOrderResolution { StartCorner = StartCorner.BottomRight,
                                                  RobotMovement = RobotMovement.Left };
            case StartOrder.BottomRightUp:
                return new StartOrderResolution { StartCorner = StartCorner.BottomRight,
                                                  RobotMovement = RobotMovement.Up };
            default:
                throw new ArgumentOutOfRangeException(nameof(order), order, "Unsupported StartOrder version 1 preset.");
            }
        }
    }
    ...
    public sealed class ImageInfo
    {
        ...
        public override string ToString()
        {
            ...
            return
                $"G{GroupId} #{ImageId} [{Row},{Column}] P{positionText} " +
                $"(x={XRobot:0.###}, y={YRobot:0.###})";
        }
    }
    ...
    public sealed class OrderOptions
    {
        ...
        public static OrderOptions FromStartOrder(StartOrder order, OrderMode mode = OrderMode.Zigzag)
        {
            ...
            return new OrderOptions { Mode = mode, StartCorner = resolved.StartCorner,
    ...
    public sealed class Bounds2D
    {
        ...
        public static Bounds2D FromPoints(IEnumerable<ImageInfo> points)
        {
            ...
            if (items.Length == 0)
                return new Bounds2D();
            return new Bounds2D { MinX = items.Min(p => p.XRobot), MinY = items.Min(p => p.YRobot),
                                  MaxX = items.Max(p => p.XRobot), MaxY = items.Max(p => p.YRobot) };
        }
    }
