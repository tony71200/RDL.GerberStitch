...
namespace GerberViewer.Stitching.RobotManager
{
    public sealed class TraversalGraph
    {
        ...
        public static TraversalGraph Build(List<List<ImageInfo>> matrix, OrderOptions options)
        {
            ...
            var graph = new TraversalGraph { Mode = options.Mode,
        ...
        {
            if (Mode == OrderMode.BranchDown)
                BuildBranchDown(matrix);
            else
                BuildRows(matrix, Mode == OrderMode.Zigzag);
        }
        ...
        private void BuildRows(List<List<ImageInfo>> matrix, bool snake)
        {
            ...
            for (int r = 0; r < matrix.Count; r++)
            {
                ...
                if (!snake && r + 1 < matrix.Count && row.FirstOrDefault() != null &&
        ...
        private void BuildBranchDown(List<List<ImageInfo>> matrix)
        {
            ...
            for (int c = 0; c < maxCols; c++)
            {
                ...
                for (int r = 0; r < matrix.Count; r++)
                {
                    if (c >= matrix[r].Count || matrix[r][c] == null)
                        continue;
                ...
                {
                    var nextHead =
                        matrix.Select(row => c + 1 < row.Count ? row[c + 1] : null).FirstOrDefault(p => p != null);
                    if (nextHead != null)
        ...
        private void BuildPathSegments()
        {
            foreach (var e in EnumerateEdges())
                PathSegments.Add(new PathSegment { FromId = e.Item1, ToId = e.Item2, Direction = e.Item3,
                                                   FromCell = CellById[e.Item1], ToCell = CellById[e.Item2] });
        }
    }
}
