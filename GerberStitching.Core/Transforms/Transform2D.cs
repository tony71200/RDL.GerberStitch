...
namespace GerberViewer.Stitching.Transforms
{
    ...
    public sealed class Transform2D
    {
        ...
        {
            get {
                return new Transform2D(new[,] { { 1d, 0d, 0d }, { 0d, 1d, 0d }, { 0d, 0d, 1d } });
            }
        }
        public static Transform2D Translation(double x, double y)
        {
            return new Transform2D(new[,] { { 1d, 0d, x }, { 0d, 1d, y }, { 0d, 0d, 1d } });
        }
        ...
        public static double[,] FromMatCv64FCopy(Mat source)
        {
            ...
            if (source.Rows != 3 || source.Cols != 3 || source.Type() != MatType.CV_64FC1)
                throw new ArgumentException("Transform Mat must be 3x3 CV_64F.", "source");
        ...
        public static HTuple ToHalconHomMat2DCopy(double[,] matrix)
        {
            ...
            if (Math.Abs(clone[2, 0]) > 1e-12 || Math.Abs(clone[2, 1]) > 1e-12 || Math.Abs(clone[2, 2] - 1d) > 1e-12)
                throw new ArgumentException(
                    "HALCON HomMat2D conversion supports affine 2D transforms with bottom row [0, 0, 1].", "matrix");
        ...
        public static double[,] FromHalconHomMat2DCopy(HTuple homMat2D)
        {
            ...
            if (homMat2D.Length != 6)
                throw new ArgumentException("HALCON HomMat2D tuple must contain 6 affine values.", "homMat2D");
            return new[,] { { homMat2D[0].D, homMat2D[1].D, homMat2D[2].D },
                            { homMat2D[3].D, homMat2D[4].D, homMat2D[5].D },
                            { 0d, 0d, 1d } };
        }
        ...
        private static double[,] ValidateAndClone(double[,] matrix)
        {
            ...
            if (matrix.GetLength(0) != 3 || matrix.GetLength(1) != 3)
                throw new ArgumentException("Transform matrix must be double[3,3].", "matrix");
            ...
            for (int row = 0; row < 3; row++)
            {
                for (int col = 0; col < 3; col++)
                {
                    ...
                    if (double.IsNaN(value) || double.IsInfinity(value))
                        throw new ArgumentException("Transform matrix contains a non-finite value.", "matrix");
                    clone[row, col] = value;
                }
            }
            return clone;
