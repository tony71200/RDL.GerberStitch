...
namespace GerberViewer.Stitching.Configuration
{
    public sealed class SampleConfigStore
    {
        ...
        public GerberSampleConfig LoadOrCreateDefault()
        {
            ...
            {
                var backup = Path.Combine(Path.GetDirectoryName(_path), Path.GetFileNameWithoutExtension(_path) + ".invalid_" + DateTime.Now.ToString("yyyyMMdd_HHmmss") + Path.GetExtension(_path));
