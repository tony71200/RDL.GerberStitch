...
namespace GerberViewer.Stitching.Configuration
{
    ...
    {
        [System.Obsolete("Neighbor production is phase-only; this value is deserialize-only and ignored.")] PyramidEcc,
        None
    }
    ...
    public sealed class AlignmentPreprocessingOptions
    {
        ...
        [Description("Linear image contrast applied only to the Direct Alignment moving/request image before OpenCV " +
                     "or HALCON matching. 100 keeps the original contrast.")]
        public double Contrast { get; set; } = 100d;
        public override string ToString()
        {
            return "Contrast " + Contrast.ToString("0.###") + "%";
        }
    }
    [TypeConverter(typeof(ExpandableObjectConverter))]
    ...
    public sealed class DirectAlignmentOptions
    {
        ...
        public override string ToString()
        {
            return CoarseMatcher + " -> " + RefinementMatcher;
        }
    }
    [TypeConverter(typeof(ExpandableObjectConverter))]
    public sealed class NeighborAlignmentOptions
    {
        public NeighborCoarseMatcherKind CoarseMatcher { get; set; } =
            NeighborCoarseMatcherKind.PyramidPhaseCorrelation;
        [Browsable(false)]
        ...
        [Description("Diagnostic-only translation disagreement threshold between accepted recovery candidates; " +
                     "selection is never fused or changed.")]
        public double MaxCandidateDisagreementPixels { get; set; } = 12;
        [Description("Diagnostic-only rotation disagreement threshold between accepted recovery candidates; " +
                     "selection is never fused or changed.")]
        public double MaxCandidateDisagreementRotationDeg { get; set; } = 0.5;
        public override string ToString()
        {
            return CoarseMatcher + " (phase-only)";
        }
    }
    [TypeConverter(typeof(ExpandableObjectConverter))]
    public sealed class RecoveryFallbackOptions
    {
        ...
        [Description("Runs neighbor matching only for tiles whose direct match failed/rejected or that are not " +
                     "stitchable. This adds matching cost only for failed tiles and may replace their failed pose " +
                     "with an accepted neighbor pose.")]
        public bool RecoverFailedTiles { get; set; } = true;
        [Category("Neighbor graph modes")]
        [DisplayName("Reconcile All Matched Tiles")]
        [Description("Builds the full eligible captured-image neighbor graph and may adjust existing poses only when " +
                     "translation/rotation residual thresholds allow it. This is the more expensive mode.")]
    ...
    public sealed class StitchingOptions
    {
        ...
        [Description("Explicitly permits the pre-Task-02 all-to-root HALCON mosaic. Its output coordinate contract " +
                     "is not verified and it must remain disabled for production.")]
