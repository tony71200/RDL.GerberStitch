...
namespace GerberViewer.Stitching.Utils
{
    public static class ImageRead
    {
        ...
        {
            public ImageReadException(string message, Exception inner = null) : base(message, inner)
            {
            }
        }
        public static Bitmap ReadBitmap(string filename)
        {
            if (string.IsNullOrWhiteSpace(filename) || !File.Exists(filename))
                throw new ImageReadException("Image not found: " + filename);
            try
            {
                using (var bmp = new Bitmap(filename)) return Ensure24bppBgr(bmp);
            }
            catch (Exception ex)
            {
                throw new ImageReadException("Failed to read image: " + filename, ex);
            }
        }
