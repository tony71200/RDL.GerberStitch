...
namespace GerberViewer.Stitching.Models
{
    ...
    public sealed class StitchingExecutionReport
    {
        ...
        // Track Save Image time separately from mosaic-construction time for the Execute Time tab.
    ...
    public enum BlankFallbackOverlapPolicy
    {
        Normal,
        PreserveExistingOverlap,
        WeightedBlend
    }
    ...
    {
        // [Tony] [Change time: 2026-07-27] [Purpose: Make runtime configuration stage-specific while retaining legacy
        // flat fields for deserialization migration.]
        ...
        public GerberViewer.Stitching.Alignment.Graph.PoseGraphOptions PoseGraph { get; set; } =
            new GerberViewer.Stitching.Alignment.Graph.PoseGraphOptions();
        ...
        [ReadOnly(true)]
        public string AlignmentMatcher
        {
            get {
                return "HalconNccMatcher";
            }
        }
        ...
        // [Tony] [Change time: 2026-08-11] [Purpose: Đổi cổng chặn ảnh preview chẩn đoán từ compile-time
        // (#if DEBUG) sang runtime, để bản Release ở trạm cũng sinh được khi người vận hành bật DebugMode.
        // Mặc định false: mọi caller cũ (GerberViewer Tab 3, F8...) giữ nguyên hành vi, và lô chạy sản xuất
        // không tốn thêm một lần đọc + warp ảnh stitch 40k x 32k.]
        public bool EmitDebugPreview { get; set; }
    ...
    // Keep one canonical PoseSource-to-OrderNodeState mapping instead of duplicating it across UI components.
    public static class OrderNodeStateMapper
    {
        public static OrderNodeState FromPoseSource(PoseSource source)
        {
            if (source == PoseSource.SampleAlignment)
                return OrderNodeState.SampleAlignOk;
            if (source == PoseSource.NeighborAlignment)
                return OrderNodeState.NeighborAlignOk;
            if (source == PoseSource.AnchorAdjusted)
                return OrderNodeState.AnchorAdjusted;
            if (source == PoseSource.Interpolated)
                return OrderNodeState.Interpolated;
            if (source == PoseSource.Manual)
                return OrderNodeState.Manual;
            if (source == PoseSource.Excluded)
                return OrderNodeState.Excluded;
            if (source == PoseSource.ExpectedGridOffset || source == PoseSource.BlankSampleExpectedPose)
                return OrderNodeState.ExpectedGridOffset;
    ...
    public sealed class ProcessingReport
    {
        ...
        // [Tony] [Change time: 2026-08-11] [Purpose: Trả đường dẫn DebugPreview ra ngoài. Tên file có
        // timestamp (DebugPreview_<yyyyHHmm>.jpg) nên Master không thể tự suy ra - bắt buộc phải truyền về.]
    ...
    public sealed class WorkflowProgress
    {
        ...
        // Return each tile result to the UI so orderPathCanvas updates while the run is in progress, not only after
        // completion.
        public WorkflowProgress(int current, int total, CapturedImageInfo image, string stage,
                                OrderNodeState? tileState)
            : this(current, total, image, stage, tileState, 0L)
        {
        }

        // [Tony] [Change time: 2026-08-10] [Purpose: Carry the elapsed time of THIS tile's stage so the host UI can
        ...
        public WorkflowProgress(int current, int total, CapturedImageInfo image, string stage,
                                OrderNodeState? tileState, long elapsedMs)
        {
            Current = current;
            Total = total;
            Image = image;
    ...
    public sealed class AlignStitchWorkflowResult
    {
        ...
        public System.Collections.Generic.IList<TileWorkflowState> States { get; set; } =
            new System.Collections.Generic.List<TileWorkflowState>();
    }
    public sealed class TileWorkflowState
    {
...
#endif
        // [Tony] [Change time: 2026-08-04] [Purpose: Preserve the pre-recovery direct pose as the pose-graph prior;
        // must be recorded in Release too, unlike DirectAlignmentTransform.]
        ...
        {
            get {
                return GerberViewer.Stitching.Alignment.Homography.IsFinite(GlobalPose);
            }
        }
        public static TileWorkflowState From(CapturedImageInfo c, double[,] pose, PoseSource source,
                                             GerberViewer.Stitching.Alignment.SampleAlignmentResult result,
                                             string reason)
        {
            var hasPose = GerberViewer.Stitching.Alignment.Homography.IsFinite(pose);
            var alignmentSucceeded =
                hasPose && (source == PoseSource.SampleAlignment || source == PoseSource.NeighborAlignment ||
                            source == PoseSource.AnchorAdjusted || source == PoseSource.Manual);
            var isFallbackPose =
                hasPose && (source == PoseSource.ExpectedGridOffset || source == PoseSource.Interpolated);
            var isStitchable = alignmentSucceeded || (hasPose && source == PoseSource.Interpolated);
            return new TileWorkflowState { OrderIndex = c.OrderIndex,
                                           Row = c.Row,
                                           Column = c.Column,
                                           GlobalPose = pose,
                                           Source = source,
        ...
        public static TileWorkflowState FromBlankSampleExpectedPose(
            CapturedImageInfo captured, SampleTileInfo tile,
            GerberViewer.Stitching.Imaging.SampleTileContentMetrics metrics, string reason)
        {
            return new TileWorkflowState { OrderIndex = captured.OrderIndex,
                                           Row = captured.Row,
                                           Column = captured.Column,
                                           GlobalPose = new[,] { { 1d, 0d, (double)tile.ExpectedX },
                                                                 { 0d, 1d, (double)tile.ExpectedY },
                                                                 { 0d, 0d, 1d } },
                                           Source = PoseSource.BlankSampleExpectedPose,
                                           Reason = reason,
                                           AlignmentSucceeded = false,
                                           IsFallbackPose = true,
                                           IsStitchable = true,
                                           SampleContent = metrics };
        }
        public StitchImagePose ToPose()
        {
            return new StitchImagePose { OrderIndex = OrderIndex,
                                         Row = Row,
                                         Column = Column,
                                         X = GlobalPose == null ? 0 : GlobalPose[0, 2],
                                         Y = GlobalPose == null ? 0 : GlobalPose[1, 2],
                                         RotationDeg = GlobalPose == null
                                                           ? 0
                                                           : System.Math.Atan2(GlobalPose[1, 0], GlobalPose[0, 0]) *
                                                                 180 / System.Math.PI,
                                         Source = Source };
        }
    }
    public sealed class ManualAlignmentRequest
    {
        ...
        public System.Collections.Generic.IList<string> Diagnostics { get; set; } =
    ...
    public enum RecoveryEdgePurpose
    {
        FailureRecovery,
    ...
    public sealed class ProcessingTileReport
    {
        ...
        public static ProcessingTileReport From(CapturedImageInfo c,
                                                GerberViewer.Stitching.Alignment.SampleAlignmentResult r,
                                                string fallback)
        {
            return new ProcessingTileReport { OrderIndex = c.OrderIndex,
                                              Row = c.Row,
                                              Column = c.Column,
                                              NccScore = r == null ? double.NaN : r.NccScore,
                                              EccCorrelation = r == null ? double.NaN : r.EccCorrelation,
                                              CoarseMatcherName = r == null ? null : r.CoarseMatcherName,
                                              CoarseRawScore = r == null ? double.NaN : r.CoarseRawScore,
                                              CoarseNormalizedConfidence =
                                                  r == null ? double.NaN : r.CoarseNormalizedConfidence,
                                              RefinementMatcherName = r == null ? null : r.RefinementMatcherName,
                                              RefinementRawScore = r == null ? double.NaN : r.RefinementRawScore,
                                              SelectedMatcherName = r == null ? null : r.SelectedMatcherName,
                                              SelectedRawScore = r == null ? double.NaN : r.SelectedRawScore,
                                              SelectedCommonScore = r == null ? double.NaN : r.SelectedCommonScore,
                                              SelectionReason = r == null ? null : r.SelectionReason,
