...
namespace GerberViewer.Stitching.Models
{
    ...
    public sealed class SampleManifest
    {
        ...
        public string ProcessedSamplePath { get; set; }
        [DataMember(Order = 13, EmitDefaultValue = false)]
        public double[][] SourceToProcessedTransform { get; set; }
        [DataMember(Order = 14, EmitDefaultValue = false)]
        public string PreprocessMode { get; set; }
        [DataMember(Order = 15)]
        public int ProcessedChannelCount { get; set; }
        [DataMember(Order = 16)]
        public int ProcessedBitDepth { get; set; }
        // Record the model-generation mode. The Worker uses it to determine whether the manifest contains .ncm/.shm
        // files or models must be created on demand. Missing values in older manifests mean OnTheFly.
    ...
    public sealed class SampleTileInfo
    {
        ...
        public int? NccModelRoiX { get; set; }
        [DataMember(Order = 11, EmitDefaultValue = false)]
        public int? NccModelRoiY { get; set; }
        [DataMember(Order = 12, EmitDefaultValue = false)]
        public int? NccModelRoiWidth { get; set; }
        [DataMember(Order = 13, EmitDefaultValue = false)]
        public int? NccModelRoiHeight { get; set; }
        [DataMember(Order = 14, EmitDefaultValue = false)]
        ...
        [DataMember(Order = 18)]
        public double SampleMinGray { get; set; }
        [DataMember(Order = 19)]
        public double SampleMaxGray { get; set; }
        [DataMember(Order = 20)]
        public double SampleMeanGray { get; set; }
        [DataMember(Order = 21)]
        public double SampleStdDevGray { get; set; }
        [DataMember(Order = 22)]
        public double SampleNonZeroPixelRatio { get; set; }
        [DataMember(Order = 23, EmitDefaultValue = false)]
        public string ShapeModelPath { get; set; }
        [DataMember(Order = 24, EmitDefaultValue = false)]
        public int? ShapeModelRoiX { get; set; }
        [DataMember(Order = 25, EmitDefaultValue = false)]
        public int? ShapeModelRoiY { get; set; }
        [DataMember(Order = 26, EmitDefaultValue = false)]
        public int? ShapeModelRoiWidth { get; set; }
        [DataMember(Order = 27, EmitDefaultValue = false)]
        public int? ShapeModelRoiHeight { get; set; }
    ...
    public sealed class SampleManifestValidationResult
    {
        ...
        public SampleManifestValidationResult()
        {
    ...
    public static class SampleManifestSerializer
    {
        public static void WriteValidated(string path, SampleManifest manifest, bool requireFiles)
        {
            ...
            if (!readbackResult.IsValid)
                throw new InvalidOperationException("Manifest readback failed: " +
                                                    string.Join(Environment.NewLine, readbackResult.Errors));
        }

        public static SampleManifest Read(string path)
        {
            using (var stream = File.OpenRead(path)) return (SampleManifest) new DataContractJsonSerializer(
                typeof(SampleManifest))
                .ReadObject(stream);
        }
    }
        ...
        public static SampleManifestValidationResult Validate(SampleManifest manifest, bool requireFiles)
        {
            ...
            if (manifest.ManifestVersion >= 2)
            {
                ...
                if (manifest.SourceToProcessedTransform != null &&
                    !IsValidTransform(manifest.SourceToProcessedTransform))
                    result.Errors.Add("SourceToProcessedTransform must be a finite 3x3 affine matrix.");
                if (requireFiles && !string.IsNullOrWhiteSpace(manifest.ProcessedSamplePath) &&
            ...
            foreach (var t in manifest.Tiles)
            {
                ...
                var requiresModel = manifest.ManifestVersion < 5 ||
                                    string.Equals(t.SampleContentClass, "Matchable", StringComparison.Ordinal);
                if (manifest.ManifestVersion >= 3 && requiresModel)
                {
                    if (string.IsNullOrWhiteSpace(t.NccModelPath))
                        ...
                        result.Errors.Add("NccModelPath unreadable/missing at order " + t.OrderIndex + ": " +
                                          t.NccModelPath);
                }
                if (manifest.ManifestVersion >= 4 && requiresModel)
                {
                    if (t.NccModelSchemaVersion <= 0)
                        result.Errors.Add("NccModelSchemaVersion missing at order " + t.OrderIndex + ".");
                    if (!t.NccModelRoiX.HasValue || !t.NccModelRoiY.HasValue || !t.NccModelRoiWidth.HasValue ||
                        !t.NccModelRoiHeight.HasValue)
                        result.Errors.Add("NCC model ROI metadata missing at order " + t.OrderIndex + ".");
                    else if (t.NccModelRoiX < 0 || t.NccModelRoiY < 0 || t.NccModelRoiWidth <= 0 ||
                             t.NccModelRoiHeight <= 0 || t.NccModelRoiX + t.NccModelRoiWidth > t.Width ||
                             t.NccModelRoiY + t.NccModelRoiHeight > t.Height)
                ...
                if (manifest.ManifestVersion >= 5)
                {
                    ...
                    if (!Enum.TryParse(t.SampleContentClass, out parsed))
                        result.Errors.Add("SampleContentClass missing/invalid at order " + t.OrderIndex + ".");
                    if (t.SampleNonZeroPixelRatio < 0 || t.SampleNonZeroPixelRatio > 1)
                        result.Errors.Add("SampleNonZeroPixelRatio outside [0,1] at order " + t.OrderIndex + ".");
                    if (exactZero && (t.SampleMinGray != 0 || t.SampleMaxGray != 0 || t.SampleNonZeroPixelRatio != 0))
                        result.Errors.Add("ExactZero metrics inconsistent at order " + t.OrderIndex + ".");
                    if (!string.IsNullOrWhiteSpace(t.ShapeModelPath))
                    {
                        if (requireFiles && !File.Exists(t.ShapeModelPath))
                            result.Errors.Add("ShapeModelPath unreadable/missing at order " + t.OrderIndex + ": " +
                                              t.ShapeModelPath);
                        if (t.ShapeModelSchemaVersion <= 0 || !t.ShapeModelRoiX.HasValue ||
                            !t.ShapeModelRoiY.HasValue || !t.ShapeModelRoiWidth.HasValue ||
                            !t.ShapeModelRoiHeight.HasValue || !t.ShapeReferenceOriginRow.HasValue ||
                            !t.ShapeReferenceOriginColumn.HasValue)
                            result.Errors.Add("Shape model metadata missing at order " + t.OrderIndex + ".");
                        else if (t.ShapeModelRoiX < 0 || t.ShapeModelRoiY < 0 || t.ShapeModelRoiWidth <= 0 ||
                                 t.ShapeModelRoiHeight <= 0 || t.ShapeModelRoiX + t.ShapeModelRoiWidth > t.Width ||
                                 t.ShapeModelRoiY + t.ShapeModelRoiHeight > t.Height)
                            result.Errors.Add("Shape model ROI is outside tile at order " + t.OrderIndex + ".");
                    }
                }
            }
