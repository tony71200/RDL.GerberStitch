...
namespace GerberViewer.Stitching.Diagnostics
{
    ...
    // [Tony] [Change time: 2026-08-01] [Purpose: Keep manifest cells, captured geometry and labels in one canonical
    // canvas contract.]
    public static class StitchedOverlayBuilder
    {
        public static IList<StitchedOverlay> Build(SampleManifest manifest, IList<CapturedImageInfo> captured,
                                                   ...
                                                   IList<RecoveryEdgeReport> recoveryEdges, double manualOffsetX,
                                                   double manualOffsetY, bool includeActualGeometry)
        {
            var result = new List<StitchedOverlay>();
            if (manifest == null || manifest.Tiles == null || states == null || captured == null)
                return result;
            ...
            foreach (var tile in manifest.Tiles.OrderBy(x => x.OrderIndex))
            {
                ...
                result.Add(new StitchedOverlay { Kind = StitchedOverlayKind.SampleCell, OrderIndex = tile.OrderIndex,
                                                 Color = "green",
                                                 Points = RectanglePoints(left, top, tile.Width, tile.Height) });
                var actual = state != null && state.HasValidPose && image != null
                                 ? TransformCorners(state.GlobalPose, image.Width, image.Height, bounds)
                                 : null;
                if (includeActualGeometry && actual != null)
                    result.Add(new StitchedOverlay { Kind = StitchedOverlayKind.ActualQuadrilateral,
                                                     OrderIndex = tile.OrderIndex,
                                                     Color = state.AlignmentSucceeded ? "yellow" : "red",
                                                     Points = actual });
                result.Add(new StitchedOverlay { Kind = StitchedOverlayKind.Label, OrderIndex = tile.OrderIndex,
                                                 ...
                                                 Text = StitchedOverlayLabelFormatter.Format(
                                                     tile, state, SelectedEdge(tile.OrderIndex, recoveryEdges)) });
            }
            return result;
        }

        public static RectangleF CalculateCanonicalBounds(IList<TileWorkflowState> states,
                                                          IDictionary<int, CapturedImageInfo> images)
        {
            var points = new List<PointF>();
            foreach (var state in states ?? new List<TileWorkflowState>())
            {
                CapturedImageInfo image;
                if (state == null || !state.HasValidPose || !images.TryGetValue(state.OrderIndex, out image))
                    continue;
                points.AddRange(TransformCorners(state.GlobalPose, image.Width, image.Height, RectangleF.Empty));
            }
            if (points.Count == 0)
                return RectangleF.Empty;
            return RectangleF.FromLTRB(points.Min(x => x.X), points.Min(x => x.Y), points.Max(x => x.X),
                                       points.Max(x => x.Y));
        }

        public static IList<PointF> TransformCorners(double[,] transform, int width, int height,
                                                     RectangleF canonicalBounds)
        {
            var source =
                new[] { new PointF(0, 0), new PointF(width, 0), new PointF(width, height), new PointF(0, height) };
            return source
                .Select(
                    p => new PointF(
                        (float)(transform[0, 0] * p.X + transform[0, 1] * p.Y + transform[0, 2] - canonicalBounds.Left),
                        (float)(transform[1, 0] * p.X + transform[1, 1] * p.Y + transform[1, 2] - canonicalBounds.Top)))
                .ToList();
        }

        private static IList<PointF> RectanglePoints(float left, float top, int width, int height)
        {
            return new[] { new PointF(left, top), new PointF(left + width, top), new PointF(left + width, top + height),
                           new PointF(left, top + height) };
        }

        private static RecoveryEdgeReport SelectedEdge(int orderIndex, IList<RecoveryEdgeReport> edges)
        {
            return (edges ?? new List<RecoveryEdgeReport>())
                .FirstOrDefault(x => x.TargetOrderIndex == orderIndex && x.Selected && x.Accepted);
        }
    }
    ...
    public static class StitchedOverlayLabelFormatter
    {
        public static string Format(SampleTileInfo tile, TileWorkflowState state, RecoveryEdgeReport selectedEdge)
        {
            ...
            if (state == null)
                return "#" + order.ToString("000", CultureInfo.InvariantCulture) + "  S:--\nSource:Missing";
            if (state.Source == PoseSource.NeighborAlignment && selectedEdge != null)
                return Header(order, double.NaN) + "\nDx:" + Signed(selectedEdge.ResidualDx) +
                       "  Dy:" + Signed(selectedEdge.ResidualDy) +
            ...
            if (state.Source == PoseSource.SampleAlignment && state.Alignment != null)
                return Header(order, state.Alignment.SelectedCommonScore) +
                       "\nDx:" + Signed(state.Alignment.TranslationX) +
                       "  Dy:" + Signed(state.Alignment.TranslationY) + "\nA:" + Signed(state.Alignment.RotationDeg) +
                       "°  Source:" + state.Source;
            return Header(order, double.NaN) + "\nSource:" + state.Source;
        }
        ...
        private static bool Invalid(double value)
        {
