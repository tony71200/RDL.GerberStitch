...
{
    // [Tony] [Change time: 2026-07-26] [Purpose: Decode each direct/recovery image at most once per workflow run.]
    internal sealed class WorkflowImageCache : IDisposable
    {
        ...
        public Mat GetMono8(string path)
        {
            return GetMono8Core(path, 100d);
        }
        ...
        {
            var key =
                path + "|contrast=" + contrastPercent.ToString("R", System.Globalization.CultureInfo.InvariantCulture);
            Mat image;
            if (!_images.TryGetValue(key, out image))
            {
                ...
                try
                {
                    ModalityAwarePreprocessor.IncreaseContrast(image, contrastPercent);
                }
                catch
                {
                    image.Dispose();
