...
namespace GerberViewer.Stitching.Alignment
{
    ...
    {
        public static NeighborAcceptanceResult Validate(MatchResult phase, Transform2D expectedTargetToAnchor,
                                                        Transform2D measuredTargetToAnchor,
                                                        Transform2D residualTransform, MatcherOptions options,
                                                        double overlapRatio)
        {
            options = options ?? new MatcherOptions();
            if (phase == null || !phase.Success)
                return Reject("No successful Pyramid Phase Correlation result.");
            if (expectedTargetToAnchor == null)
                return Reject("ExpectedTargetToAnchorTransform is missing.");
            if (measuredTargetToAnchor == null)
                return Reject("MeasuredTargetToAnchorTransform is missing.");
            if (residualTransform == null)
                return Reject("ResidualTransform is missing.");
            ...
            if (Math.Abs(rotationDeg) > 1e-9)
                return Reject("Phase-only pair transform must be translation-only.");
            var residual = residualTransform.ToArray();
            var canonicalResidual =
                NeighborTransformModel.Residual(expectedTargetToAnchor, measuredTargetToAnchor).ToArray();
            if (Math.Abs(residual[0, 2] - canonicalResidual[0, 2]) > 1e-6 ||
                Math.Abs(residual[1, 2] - canonicalResidual[1, 2]) > 1e-6)
        ...
        public static bool IsAccepted(object pairResult)
        {
            if (pairResult == null)
                return false;
            var eval = pairResult.GetType()
                           .GetProperty("Eval", BindingFlags.Instance | BindingFlags.Public)
                           ?.GetValue(pairResult, null);
            if (eval == null)
                return false;
            var isMatch = eval.GetType()
