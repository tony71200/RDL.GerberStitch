...
namespace GerberViewer.Stitching.Alignment
{
    ...
    public sealed class AlignStitchWorkflowService
    {
        ...
        internal Task<AlignStitchWorkflowResult> RunSimpleAsync(
            AlignStitchConfig config,
            SampleManifest manifest,
            IList<CapturedImageInfo> captured,
            IProgress<WorkflowProgress> progress,
            CancellationToken cancellationToken)
        {
            return Task.Run(() => RunCore(config ?? new AlignStitchConfig(), manifest, captured,
            ...
            bool simpleWorkflow = false)
        {
            ...
            report.Messages.Add("Neighbor graph modes: FailureRecovery=" + (config.Recovery.RecoverFailedTiles ? "enabled" : "disabled") + "; FullPoseReconciliation=" + (config.Recovery.ReconcileAllMatchedTiles ? "enabled" : "disabled") + ".");
            ...
            report.Messages.Add("Transform contract: HalconNccMatcher returns MovingImage-to-ReferenceImage transforms. Direct alignment warps each captured MovingImage into its reference tile coordinate system, then composes the tile ExpectedX/ExpectedY offset for global stitching.");
            ...
            using (_imageCache = new WorkflowImageCache())
            using (var aligner = _alignerFactory == null ? null : _alignerFactory())
            {
                for (int i = 0; i < ordered.Count; i++)
                {
                    ...
                    {
                        var reason = "Direct alignment failed; retained the initial expected tile coordinate for simple stitching.";
                ...
                if (!simpleWorkflow && config.PoseGraph.Enabled && !config.Recovery.ReconcileAllMatchedTiles)
                {
                    ...
                    report.Warnings.Add("PoseGraph.Enabled requires Recovery.ReconcileAllMatchedTiles; it was off and has been enabled for this run.");
                }
                swNeighbor.Start();
                ...
                Mark(report, "Neighbor Graph", swNeighbor.ElapsedMilliseconds,
                    (!simpleWorkflow && config.Recovery.ReconcileAllMatchedTiles) ? ("tiles=" + ordered.Count) : "skipped (ReconcileAllMatchedTiles=false)");
            }
            _imageCache = null;
            ...
            swValidate.Stop();
            var offsetMessage = string.Format(
                System.Globalization.CultureInfo.InvariantCulture,
                "Configured direct-alignment manual offset: X={0:R}, Y={1:R} pixels.",
                config.DirectAlignment.ManualOffsetX,
                config.DirectAlignment.ManualOffsetY);
            ...
            AddNeighborModeSummary(report, RecoveryEdgePurpose.FullPoseReconciliation, config.Recovery.ReconcileAllMatchedTiles,
            ...
            if (!report.Succeeded)
                report.Warnings.Add("One or more tiles could not be aligned; black placeholders were written at their expected grid positions.");
            if (!string.IsNullOrWhiteSpace(config.OutputPath))
            {
                ...
                if (config.PoseGraph.Enabled)
                {
                    ...
                    foreach (var w in report.PoseGraph.Warnings) report.Warnings.Add(w);
                    report.Messages.Add("PoseGraph: enabled; DirectPoseOutlierCorrector skipped (pose graph supersedes median-angle correction).");
                    progress?.Report(new WorkflowProgress(ordered.Count, ordered.Count, null,
                ...
                {
                    // [Codex] [Change time: 2026-08-03] [Purpose: Correct anisotropic direct-pose outliers before the global safety validator and stitching boundary.]
                    ...
                    foreach (var warning in report.DirectPoseCorrection.Warnings) report.Warnings.Add(warning);
                    progress?.Report(new WorkflowProgress(ordered.Count, ordered.Count, null,
                ...
                swPoseGraph.Stop();
                Mark(report, "Pose Graph Optimizer", swPoseGraph.ElapsedMilliseconds, config.PoseGraph.Enabled
                ...
                report.GlobalPoseValidation = poseValidator.Validate(config, manifest, ordered, outputStates, report.RecoveryEdges);
                foreach (var diagnostic in report.GlobalPoseValidation.Tiles)
                {
                    ...
                    if (tileReport != null) tileReport.GlobalPoseDiagnostic = diagnostic;
                }
                progress?.Report(new WorkflowProgress(ordered.Count, ordered.Count, null,
                    "GlobalPose summary: valid=" + report.GlobalPoseValidation.IsValid + "; tiles=" + report.GlobalPoseValidation.Tiles.Count + "; outliers=" + report.GlobalPoseValidation.Errors.Count));
                swValidate.Stop();
                Mark(report, "Validate", swValidate.ElapsedMilliseconds, "valid=" + report.GlobalPoseValidation.IsValid + "; outliers=" + report.GlobalPoseValidation.Errors.Count);
                ...
                var saveElapsedMs = report.StitchingExecution != null ? report.StitchingExecution.SaveElapsedMilliseconds : 0;
                Mark(report, "Stitching", Math.Max(0, swStitch.ElapsedMilliseconds - saveElapsedMs),
                    report.StitchingExecution != null ? report.StitchingExecution.EffectiveEngine.ToString() : string.Empty);
                Mark(report, "Save Image", saveElapsedMs,
                    string.IsNullOrWhiteSpace(report.FinalOutputPath) ? string.Empty : Path.GetFileName(report.FinalOutputPath));
                // [Codex] [Change time: 2026-08-04] [Purpose: Surface the HALCON blending-ignored diagnostic as a run warning, not just a buried matrix diagnostic line.]
                if (report.StitchingExecution != null)
                    foreach (var diagnostic in report.StitchingExecution.MatrixDiagnostics.Where(d => d.Contains("has no blending parameter")))
                        report.Warnings.Add(diagnostic);
#if DEBUG
                try
                {
                    var debugPreview = DebugHtmlReportWriter.WriteStitchedPreview(config.OutputPath, report.FinalOutputPath,
                        manifest, ordered, outputStates, config.DirectAlignment.ManualOffsetX, config.DirectAlignment.ManualOffsetY);
                    if (string.IsNullOrWhiteSpace(debugPreview))
                        report.Warnings.Add("DEBUG stitched preview was not created because the stitched image or processed sample mask was unavailable.");
                }
                catch (Exception debugPreviewError)
                {
                    // A best-effort DEBUG artifact must never turn a successfully stitched run into a failed run.
                    report.Warnings.Add("DEBUG stitched preview was skipped: " + debugPreviewError.Message);
                }
#endif
            }
            else
        ...
        private static void AddNeighborModeSummary(ProcessingReport report, RecoveryEdgePurpose purpose, bool enabled, int adjusted)
        {
        ...
        private static void ReportMatcherResults(IProgress<WorkflowProgress> progress, CapturedImageInfo image, SampleAlignmentResult result, string section)
        {
            if (result == null) return;
            PopulatePoseComponents(result);
            if (progress == null) return;
            if (!string.IsNullOrWhiteSpace(result.CoarseMatcherName))
                progress.Report(new WorkflowProgress(image.OrderIndex + 1, 0, image,
                    string.Format("Alignment/{0}/Coarse: {1}", section, FormatMatchResult(result.CoarseMatcherName, result.Success, result.CoarseRawScore, result))));
            if (!string.IsNullOrWhiteSpace(result.RefinementMatcherName))
                progress.Report(new WorkflowProgress(image.OrderIndex + 1, 0, image,
                    string.Format("Alignment/{0}/Refinement: {1}", section, FormatMatchResult(result.RefinementMatcherName, result.Success, result.RefinementRawScore, result))));
            progress.Report(new WorkflowProgress(image.OrderIndex + 1, 0, image,
                string.Format("Alignment/{0}/Result: success={1}, selected={2}, Dx={3:0.###}, Dy={4:0.###}, Ang={5:0.###}, rejection={6}", section, result.Success, result.PipelineStage ?? "-", result.TranslationX, result.TranslationY, result.RotationDeg, result.RejectionReason ?? "-")));
        }

        private static string FormatMatchResult(string matcherName, bool success, double rawScore, SampleAlignmentResult alignment)
        {
            return new MatchResult
            {
                ...
                FailureReason = success ? MatchFailureReason.None : MatchFailureReason.RuntimeFailure,
        ...
        {
            if (result == null || !result.Success || !Homography.IsFinite(result.CapturedToSampleTransform)) return;
            var transform = result.CapturedToSampleTransform;
            if (double.IsNaN(result.TranslationX)) result.TranslationX = transform[0, 2];
            if (double.IsNaN(result.TranslationY)) result.TranslationY = transform[1, 2];
            if (double.IsNaN(result.RotationDeg)) result.RotationDeg = Math.Atan2(transform[1, 0], transform[0, 0]) * 180.0 / Math.PI;
            if (double.IsNaN(result.Scale)) result.Scale = Math.Sqrt(transform[0, 0] * transform[0, 0] + transform[1, 0] * transform[1, 0]);
        }

        private TileWorkflowState SolveDirect(AlignStitchConfig config, SampleTileInfo tile, CapturedImageInfo cap, ISampleAligner aligner, ProcessingReport report, CancellationToken ct, bool allowAutomaticBlankPlacement)
        {
            ct.ThrowIfCancellationRequested();
            var metrics = GetSampleContent(config, tile);
            if (allowAutomaticBlankPlacement && (metrics.ContentClass == SampleTileContentClass.ExactZero || metrics.ContentClass == SampleTileContentClass.LowTexture) && config.AutoPlaceExactZeroSample)
            {
                var reason = "Exact-zero sample has no measurable alignment content; placed at deterministic expected grid pose.";
            ...
            using (var img = LoadBitmap(cap.FilePath))
            {
                ...
                if (r.Success)
                {
                    var global = Multiply(DirectTileTranslation(config, tile),
                        r.CapturedToSampleTransform);
                    var directState = WithContent(TileWorkflowState.From(cap, global, PoseSource.SampleAlignment, r, null), metrics);
                    // [Codex] [Change time: 2026-08-04] [Purpose: Preserve the original direct pose as the pose-graph prior, in both Debug and Release.]
        ...
        private TileWorkflowState SolveDirectWithMatcherFactory(AlignStitchConfig config, SampleTileInfo tile, CapturedImageInfo cap, ProcessingReport report, CancellationToken ct, SampleTileContentMetrics metrics)
        {
            var pipeline = new MatcherPipeline(_matcherFactory, ForwardMatcherResult);
            MatcherKind? coarse;
            try { coarse = AlignStitchConfigMapper.ToDirectCoarseMatcherKind(config); }
            catch (NotSupportedException ex) { return WithContent(TileWorkflowState.From(cap, null, PoseSource.Failed, null, ex.Message), metrics); }
            var refinement = AlignStitchConfigMapper.ToDirectRefinementMatcherKind(config);
            var directRequest = new MatchRequest
            {
                ReferenceImage = _imageCache.GetMono8(tile.ExpectedPath),
                MovingImage = _imageCache.GetDirectMovingMono8(cap.FilePath, config.DirectAlignment.Preprocessing.Contrast),
                ...
                ReferenceShapeModelRoi = ToShapeRoi(tile), ShapeReferenceOriginRow = tile.ShapeReferenceOriginRow, ShapeReferenceOriginColumn = tile.ShapeReferenceOriginColumn, ShapeModelSchemaVersion = tile.ShapeModelSchemaVersion, ShapeOptions = config.DirectAlignment.Shape,
            ...
            var result = pipeline.MatchDirect(directRequest, coarse, refinement, config.DirectAlignment.Policy.AllowCoarseOnlyAcceptance, config.DirectAlignment.Policy.AllowRefinementFromExpectedWhenCoarseFails, ct);
            DirectCandidateEvaluation evaluation = null;
            if (config.DirectAlignment.Evaluation.ReportOnlyEnabled || config.DirectAlignment.Evaluation.SelectionPolicy == DirectSelectionPolicy.WeightedCommonMetric)
            {
                evaluation = new DirectCandidateEvaluator().Evaluate(new DirectCandidateEvaluationRequest
                {
            ...
            var alignment = new SampleAlignmentResult
            {
                ...
                CapturedToSampleTransform = result.Success && match != null ? match.MovingToReferenceTransform.ToArray() : null,
                NccScore = coarse == MatcherKind.HalconNcc && result.CoarseCandidate != null ? result.CoarseCandidate.RawScore : double.NaN,
                EccCorrelation = refinement == MatcherKind.PyramidEcc && result.RefinementCandidate != null ? result.RefinementCandidate.RawScore : double.NaN,
                CoarseMatcherName = result.CoarseCandidate == null ? null : result.CoarseCandidate.MatcherName, CoarseRawScore = result.CoarseCandidate == null ? double.NaN : result.CoarseCandidate.RawScore, CoarseNormalizedConfidence = result.CoarseCandidate == null ? double.NaN : result.CoarseCandidate.NormalizedConfidence, RefinementMatcherName = result.RefinementCandidate == null ? null : result.RefinementCandidate.MatcherName, RefinementRawScore = result.RefinementCandidate == null ? double.NaN : result.RefinementCandidate.RawScore,
                SelectedMatcherName = match == null ? null : match.MatcherName, SelectedRawScore = match == null ? double.NaN : match.RawScore, SelectedCommonScore = result.SelectedCommonScore, SelectionReason = result.SelectionReason,
                CandidateEvaluation = evaluation,
                NccFailureReason = result.CoarseCandidate == null || result.CoarseCandidate.Success ? null : result.CoarseCandidate.FailureReason + ": " + result.CoarseCandidate.FailureMessage,
                EccFailureReason = result.RefinementCandidate == null || result.RefinementCandidate.Success ? null : result.RefinementCandidate.FailureReason + ": " + result.RefinementCandidate.FailureMessage,
            ...
            if (!alignment.Success) return WithContent(TileWorkflowState.From(cap, null, PoseSource.Failed, alignment, alignment.RejectionReason), metrics);
            var directGlobal = Multiply(DirectTileTranslation(config, tile), alignment.CapturedToSampleTransform);
            var directMatcherState = WithContent(TileWorkflowState.From(cap, directGlobal, PoseSource.SampleAlignment, alignment, null), metrics);
            // [Codex] [Change time: 2026-08-04] [Purpose: Preserve the original direct pose as the pose-graph prior, in both Debug and Release.]
        ...
        {
            return Translation(tile.ExpectedX + config.DirectAlignment.ManualOffsetX, tile.ExpectedY + config.DirectAlignment.ManualOffsetY);
        }

        private static SampleAlignmentMethod ToDirectAlignmentMethod(MatcherKind? coarse, MatcherKind? refinement, MatcherPipelineResult result)
        {
            var refinementAccepted = result != null && result.RefinementCandidate != null && result.RefinementCandidate.Success && object.ReferenceEquals(result.SelectedCandidate, result.RefinementCandidate);
            if (refinementAccepted && refinement == MatcherKind.PyramidPhaseCorrelation) return SampleAlignmentMethod.PyramidPhaseCorrelation;
            if (refinementAccepted && refinement == MatcherKind.PyramidEcc)
            {
                if (coarse == MatcherKind.HalconNcc) return SampleAlignmentMethod.NccThenPyramidEcc;
                if (coarse == MatcherKind.HalconShapeModel) return SampleAlignmentMethod.ShapeThenPyramidEcc;
                return SampleAlignmentMethod.PyramidEcc;
            }
            if (coarse == MatcherKind.HalconShapeModel) return SampleAlignmentMethod.HalconShapeModel;
            if (coarse == MatcherKind.PyramidEcc) return SampleAlignmentMethod.PyramidEcc;
            if (coarse == MatcherKind.PyramidPhaseCorrelation) return SampleAlignmentMethod.PyramidPhaseCorrelation;
            ...
            bool includeSuccessor)
        {
            if (AlignStitchConfigMapper.ToNeighborCoarseMatcherKind(config).HasValue)
            {
                ...
                {
                    ct.ThrowIfCancellationRequested();
                    var edge = MatchNeighborEdge(config, tile, cap, candidate.Anchor.OrderIndex,
                        capturedByOrder, tileByOrder, RecoveryEdgePurpose.FailureRecovery, ct);
        ...
        private TileWorkflowState ResolveFailedWithoutNeighbor(AlignStitchConfig config, SampleTileInfo tile,
            CapturedImageInfo cap, ProcessingReport report, CancellationToken ct)
        {
            report.Warnings.Add(TilePrefix(cap) + "Failed-tile neighbor recovery disabled; applying manual/excluded/failure policy.");
            var expected = Translation(tile.ExpectedX, tile.ExpectedY);
            if (config.Recovery.EnableManualAlignment && _manualProvider != null)
            {
                var manual = _manualProvider.RequestManualAlignment(new ManualAlignmentRequest
                {
                    Captured = cap, SampleTile = tile, ExpectedGlobalPose = expected,
                    Diagnostics = report.TileReports.Where(x => x.OrderIndex == cap.OrderIndex).Select(x => x.RejectionReason ?? x.FallbackReason).ToList()
                }, ct);
                if (manual != null)
                {
                    if (manual.CancelRun) throw new OperationCanceledException("Manual alignment cancelled the run.", ct);
            ...
            return TileWorkflowState.From(cap, null, PoseSource.Failed, null, "Direct alignment failed and failed-tile neighbor recovery is disabled.");
        }

        private static TileWorkflowState CompleteRecoveryFromAnchor(
            CapturedImageInfo target, RecoveryCandidate candidate, RecoveryEdgeReport edge,
            ProcessingReport report)
        {
            ...
            edge.Selected = true;
            edge.InheritedAnchorRotationDeg = Math.Atan2(candidate.Anchor.GlobalPose[1, 0], candidate.Anchor.GlobalPose[0, 0]) * 180 / Math.PI;
            edge.ComposedIntoGlobalPose = true;
            edge.CompositionAnchorGlobalPose = CloneMatrix(candidate.Anchor.GlobalPose);
            var alignment = new SampleAlignmentResult
            {
                Success = true, Method = SampleAlignmentMethod.PyramidPhaseCorrelation,
                CapturedToSampleTransform = edge.TargetToAnchorTransform,
                NccScore = edge.PhaseScore, 
                EccCorrelation = double.NaN,
                PipelineStage = edge.Matcher, PreprocessingVariant = "neighbor-overlap-" + edge.Direction,
                TranslationX = edge.TargetToAnchorTransform[0, 2], TranslationY = edge.TargetToAnchorTransform[1, 2],
                RotationDeg = Math.Atan2(edge.TargetToAnchorTransform[1, 0], edge.TargetToAnchorTransform[0, 0]) * 180 / Math.PI,
                Scale = 1, OverlapRatio = edge.OverlapRatio
            };
            edge.RankingReason = "Selected rank 1: " + NeighborCandidateRanker.RankingTuple(edge) + ".";
            AddReport(report, target, alignment, "Recovered from selected anchor OrderIndex " + candidate.Anchor.OrderIndex + " via " + edge.Direction + "; " + edge.RankingReason);
            return TileWorkflowState.From(target, global, PoseSource.NeighborAlignment, alignment, edge.Reason + " " + edge.RankingReason);
        }

        private static void PopulateCandidateGlobalPosesAndConsistency(AlignStitchConfig config,
            IList<Tuple<RecoveryCandidate, RecoveryEdgeReport>> measured, IList<RecoveryEdgeReport> ranked,
            ProcessingReport report, CapturedImageInfo target)
        {
            foreach (var item in measured.Where(x => x.Item2.Accepted))
                item.Item2.CandidateGlobalPose = new Transform2D(item.Item1.Anchor.GlobalPose)
                    .Multiply(new Transform2D(item.Item2.MeasuredTargetToAnchorTransform)).ToArray();
            for (var i = 0; i < ranked.Count; i++)
                for (var j = i + 1; j < ranked.Count; j++)
                {
                    ...
                    ranked[i].MaximumTranslationDisagreement = Maximum(ranked[i].MaximumTranslationDisagreement, disagreement.Item1);
                    ranked[j].MaximumTranslationDisagreement = Maximum(ranked[j].MaximumTranslationDisagreement, disagreement.Item1);
                    ranked[i].MaximumRotationDisagreementDeg = Maximum(ranked[i].MaximumRotationDisagreementDeg, disagreement.Item2);
                    ranked[j].MaximumRotationDisagreementDeg = Maximum(ranked[j].MaximumRotationDisagreementDeg, disagreement.Item2);
                    ...
                    {
                        var warning = "Candidate pose disagreement: anchors " + ranked[i].AnchorOrderIndex + " and " + ranked[j].AnchorOrderIndex +
                            ", translation=" + disagreement.Item1.ToString("0.###", System.Globalization.CultureInfo.InvariantCulture) +
                            ", rotationDeg=" + disagreement.Item2.ToString("0.###", System.Globalization.CultureInfo.InvariantCulture) +
                            ". Rank 1 remains selected; no fusion.";
                        ranked[i].ConsistencyWarning = warning; ranked[j].ConsistencyWarning = warning;
                        report.Warnings.Add(TilePrefix(target) + warning);
                    }
                }
        }

        private static double Maximum(double current, double value) { return double.IsNaN(current) ? value : Math.Max(current, value); }
        ...
        private RecoveryEdgeReport MatchNeighborEdge(
            AlignStitchConfig config, SampleTileInfo targetTile, CapturedImageInfo target,
            int anchorOrderIndex, IDictionary<int, CapturedImageInfo> capturedByOrder,
            IDictionary<int, SampleTileInfo> tileByOrder, RecoveryEdgePurpose purpose,
            CancellationToken ct)
        {
            CapturedImageInfo anchorCap;
            SampleTileInfo anchorTile;
            if (!capturedByOrder.TryGetValue(anchorOrderIndex, out anchorCap) || !tileByOrder.TryGetValue(anchorOrderIndex, out anchorTile)) return null;
            var direction = NeighborTransformModel.Direction(anchorTile, targetTile);
            if (direction == null) return null;
#pragma warning disable 618
            var legacyRefinementWarning = config.NeighborAlignment.RefinementMatcher == NeighborRefinementMatcherKind.PyramidEcc
                ? " Legacy Neighbor RefinementMatcher=PyramidEcc is deprecated and ignored; production is phase-only." : string.Empty;
#pragma warning restore 618
            if (!AlignStitchConfigMapper.ToNeighborCoarseMatcherKind(config).HasValue)
                return CreateRecoveryEdge(anchorOrderIndex, target.OrderIndex, direction, "None", "anchor=" + anchorOrderIndex + ", target=" + target.OrderIndex + ", direction=" + direction + ": Neighbor coarse matcher is disabled.", null, null, null, null, 0, purpose, false);
            ...
            using (var anchorRoi = CropCopy(anchorMat, anchorOverlapRoi))
            using (var targetRoi = CropCopy(targetMat, targetOverlapRoi))
            {
                double sampleForegroundRatio;
                double sampleEdgeDensity;
                CalculateSampleOverlapMetrics(targetTile, anchorTile, direction, out sampleForegroundRatio, out sampleEdgeDensity);
                ...
                MatchResult phase;
                using (var phaseMatcher = _matcherFactory.Create(MatcherKind.PyramidPhaseCorrelation, ForwardMatcherResult))
                ...
                { 
                    var failedEdge = CreateRecoveryEdge(
                        ...
                        target.OrderIndex, 
                        direction, 
                        "PyramidPhaseCorrelationMatcher",
                        "anchor=" + anchorOrderIndex + ", target=" + target.OrderIndex + ", direction=" + direction + ": " + phase.FailureReason + ": " + phase.FailureMessage + legacyRefinementWarning,
                        NeighborTransformModel.ExpectedTargetToAnchor(anchorTile, targetTile).ToArray(),
                        null,
                        null,
                        phase,
                        overlap, purpose, false);
                ...
                var acceptance = NeighborMatchAcceptance.Validate(
                    phase, expected, targetToAnchor, residual, options, overlap);
                var residualMatrix = residual.ToArray();
                var contextualReason = "anchor=" + anchorOrderIndex + ", target=" + target.OrderIndex + ", direction=" + direction +
                    ": Residual Dx=" + residualMatrix[0, 2].ToString("0.###", System.Globalization.CultureInfo.InvariantCulture) +
                    ", Dy=" + residualMatrix[1, 2].ToString("0.###", System.Globalization.CultureInfo.InvariantCulture) + ". " + acceptance.Reason + legacyRefinementWarning;
                var acceptedEdge = CreateRecoveryEdge(
                    anchorOrderIndex,
                    target.OrderIndex, 
                    direction, 
                    "PyramidPhaseCorrelationMatcher",
                    contextualReason,
                    expected.ToArray(), targetToAnchor.ToArray(), residualMatrix,
                    phase,
                    overlap, purpose, acceptance.IsMatch);
                acceptedEdge.SampleForegroundRatio = sampleForegroundRatio;
                acceptedEdge.SampleEdgeDensity = sampleEdgeDensity;
                acceptedEdge.ResidualNorm = Math.Sqrt(residualMatrix[0, 2] * residualMatrix[0, 2] + residualMatrix[1, 2] * residualMatrix[1, 2]);
                return acceptedEdge;
            }
        }
            ...
            string direction, out double foregroundRatio, out double edgeDensity)
        {
            ...
            using (var crop = CropCopy(sample, roi))
        ...
        private void ReconcileNeighborGraph(AlignStitchConfig config, Dictionary<int, TileWorkflowState> solved,
            IList<CapturedImageInfo> ordered, IDictionary<int, CapturedImageInfo> capturedByOrder,
            ...
            IProgress<WorkflowProgress> progress, CancellationToken ct)
        {
            ...
            var eligible = ordered.Where(x => x != null && !string.IsNullOrWhiteSpace(x.FilePath) && File.Exists(x.FilePath) &&
                solved.ContainsKey(x.OrderIndex) && solved[x.OrderIndex].Source != PoseSource.Excluded).ToList();
            foreach (var a in eligible.OrderBy(x => x.OrderIndex))
                foreach (var b in eligible.Where(x => x.OrderIndex > a.OrderIndex && Math.Abs(x.Row - a.Row) + Math.Abs(x.Column - a.Column) == 1))
                    pairs.Add(Tuple.Create(a, b)); // canonical order prevents the reverse duplicate

            foreach (var pair in pairs)
            {
                ct.ThrowIfCancellationRequested();
                var edge = MatchNeighborEdge(config, tileByOrder[pair.Item2.OrderIndex], pair.Item2,
                    pair.Item1.OrderIndex, capturedByOrder, tileByOrder, RecoveryEdgePurpose.FullPoseReconciliation, ct);
                if (edge == null) continue;
                if (solved[pair.Item1.OrderIndex].HasValidPose)
                    edge.InheritedAnchorRotationDeg = Math.Atan2(solved[pair.Item1.OrderIndex].GlobalPose[1, 0], solved[pair.Item1.OrderIndex].GlobalPose[0, 0]) * 180 / Math.PI;
                if (edge.Accepted && solved[pair.Item1.OrderIndex].HasValidPose && solved[pair.Item2.OrderIndex].HasValidPose)
                {
                    ...
                    {
                        edge.Accepted = false;
                        edge.Reason = "Rejected by cycle/direct-pose closure check; residual=" + closure.Item1.ToString("0.###", System.Globalization.CultureInfo.InvariantCulture) + " px.";
                    }
                }
                report.RecoveryEdges.Add(edge);
                progress?.Report(new WorkflowProgress(pair.Item2.OrderIndex + 1, ordered.Count, pair.Item2,
                    "Full pose reconciliation: " + pair.Item1.OrderIndex + "->" + pair.Item2.OrderIndex + " " + edge.Reason));
            }

            // [Codex] [Change time: 2026-08-04] [Purpose: When the pose graph optimizer is enabled, only measure edges here; it supersedes single-path anchor selection/propagation below.]
            if (config.PoseGraph.Enabled) return;

            var usable = report.RecoveryEdges.Where(x => x.Purpose == RecoveryEdgePurpose.FullPoseReconciliation && x.Accepted && x.TargetToAnchorTransform != null).ToList();
            var direct = solved.Values.Where(x => x.Source == PoseSource.SampleAlignment && x.HasValidPose)
                .OrderByDescending(DirectConfidence).ThenBy(x => x.OrderIndex).FirstOrDefault();
            if (direct == null)
                direct = solved.Values.Where(x => x.IsStitchable && x.HasValidPose).OrderBy(x => x.OrderIndex).FirstOrDefault();
            ...
            {
                int bestNode = -1; double bestQuality = -1; double[,] bestPose = null;
                foreach (var edge in usable)
                {
                    double[,] candidatePose = null; int node = -1;
                    if (poses.ContainsKey(edge.AnchorOrderIndex) && pending.Contains(edge.TargetOrderIndex))
                    { node = edge.TargetOrderIndex; candidatePose = Multiply(poses[edge.AnchorOrderIndex], edge.TargetToAnchorTransform); }
                    else if (poses.ContainsKey(edge.TargetOrderIndex) && pending.Contains(edge.AnchorOrderIndex))
                    { node = edge.AnchorOrderIndex; candidatePose = Multiply(poses[edge.TargetOrderIndex], new Transform2D(edge.TargetToAnchorTransform).Invert().ToArray()); }
                    if (node < 0) continue;
                    var q = Math.Min(qualities.ContainsKey(edge.AnchorOrderIndex) ? qualities[edge.AnchorOrderIndex] : qualities[edge.TargetOrderIndex], Math.Max(1e-9, edge.Quality));
                    if (q > bestQuality || (Math.Abs(q - bestQuality) < 1e-12 && node < bestNode)) { bestNode = node; bestQuality = q; bestPose = candidatePose; }
                }
                if (bestNode < 0) break;
                poses[bestNode] = bestPose; qualities[bestNode] = bestQuality; pending.Remove(bestNode);
            }

            foreach (var entry in poses.Where(x => x.Key != direct.OrderIndex))
            {
                var state = solved[entry.Key];
                if (state.Source == PoseSource.Excluded) continue;
                if (!state.HasValidPose || !state.IsStitchable)
                {
                    var reconciled = WithContent(TileWorkflowState.From(capturedByOrder[entry.Key], entry.Value, PoseSource.NeighborAlignment, state.Alignment, "Connected to deterministic anchor " + direct.OrderIndex + " by accepted reconciliation edges."), state.SampleContent);
#if DEBUG
                ...
                if (residual.Item1 <= config.NeighborAlignment.MaxDirectPoseAdjustmentPixels && residual.Item2 <= config.NeighborAlignment.MaxDirectPoseAdjustmentRotationDeg && residual.Item1 > 1e-6)
                {
                    var tileReport = report.TileReports.LastOrDefault(x => x.OrderIndex == entry.Key);
                    if (tileReport == null) { tileReport = new ProcessingTileReport { OrderIndex = entry.Key, Row = state.Row, Column = state.Column }; report.TileReports.Add(tileReport); }
                    tileReport.OriginalGlobalPose = CloneMatrix(state.GlobalPose); tileReport.AdjustedGlobalPose = CloneMatrix(entry.Value);
                    tileReport.AdjustmentTranslationResidual = residual.Item1; tileReport.AdjustmentRotationResidualDeg = residual.Item2;
                    tileReport.AdjustmentReason = "Accepted highest-quality path from anchor " + direct.OrderIndex + ".";
                    state.GlobalPose = entry.Value; state.Source = PoseSource.AnchorAdjusted; state.Reason = tileReport.AdjustmentReason;
                }
            }
        }
        ...
        {
            if (state.Alignment == null) return double.NegativeInfinity;
            if (!double.IsNaN(state.Alignment.CoarseNormalizedConfidence)) return state.Alignment.CoarseNormalizedConfidence;
            if (!double.IsNaN(state.Alignment.EccCorrelation)) return state.Alignment.EccCorrelation;
            return state.Alignment.NccScore;
        }

        private static Tuple<double, double> PoseResidual(double[,] a, double[,] b)
        {
            var dx = a[0, 2] - b[0, 2]; var dy = a[1, 2] - b[1, 2];
        ...
        private static double[,] CloneMatrix(double[,] value) { return value == null ? null : (double[,])value.Clone(); }

        private static void ValidateGlobalPoses(AlignStitchConfig config, Dictionary<int, TileWorkflowState> solved, IDictionary<int, SampleTileInfo> tiles, ProcessingReport report)
        {
            foreach (var state in solved.Values.Where(x => x.IsStitchable))
            {
                ...
                var rotation = Homography.IsFinite(p) ? Math.Abs(Math.Atan2(p[1, 0], p[0, 0]) * 180 / Math.PI) : double.NaN;
                var tile = tiles[state.OrderIndex];
                var inCanvas = Homography.IsFinite(p) && Math.Abs(p[0, 2]) < 1000000000d && Math.Abs(p[1, 2]) < 1000000000d && tile.Width > 0 && tile.Height > 0;
                if (!Homography.IsFinite(p) || scale < config.NeighborAlignment.Acceptance.MinScale || scale > config.NeighborAlignment.Acceptance.MaxScale || rotation > 180 || !inCanvas)
                    throw new InvalidOperationException("Invalid reconciled GlobalPose for OrderIndex " + state.OrderIndex + ".");
            }
            var accepted = report.RecoveryEdges.Where(x => x.Accepted).ToList();
            var connected = new HashSet<int>(solved.Values.Where(x => x.Source == PoseSource.SampleAlignment || x.Source == PoseSource.Manual || x.Source == PoseSource.BlankSampleExpectedPose).Select(x => x.OrderIndex));
            bool changed;
            do
            {
                changed = false;
                ...
                {
                    if (connected.Contains(edge.AnchorOrderIndex) && connected.Add(edge.TargetOrderIndex)) changed = true;
                    if (connected.Contains(edge.TargetOrderIndex) && connected.Add(edge.AnchorOrderIndex)) changed = true;
                }
            } while (changed);
            foreach (var state in solved.Values.Where(x => x.Source == PoseSource.NeighborAlignment && x.IsStitchable))
                if (!connected.Contains(state.OrderIndex))
                    throw new InvalidOperationException("Neighbor-aligned tile is disconnected from the accepted edge graph: " + state.OrderIndex + ".");
        }

        private static Rect AnchorRoi(Mat anchor, SampleTileInfo anchorTile, SampleTileInfo targetTile, string direction)
        {
            // The overlap size comes from manifest tile geometry, never from an arbitrary image fraction.
            var w = Math.Max(1, Math.Min(anchor.Cols, Math.Min(anchorTile.ExpectedX + anchorTile.Width, targetTile.ExpectedX + targetTile.Width) - Math.Max(anchorTile.ExpectedX, targetTile.ExpectedX)));
            var h = Math.Max(1, Math.Min(anchor.Rows, Math.Min(anchorTile.ExpectedY + anchorTile.Height, targetTile.ExpectedY + targetTile.Height) - Math.Max(anchorTile.ExpectedY, targetTile.ExpectedY)));
        ...
        private static Rect TargetRoi(Mat target, SampleTileInfo anchorTile, SampleTileInfo targetTile, string direction)
        {
            var w = Math.Max(1, Math.Min(target.Cols, Math.Min(anchorTile.ExpectedX + anchorTile.Width, targetTile.ExpectedX + targetTile.Width) - Math.Max(anchorTile.ExpectedX, targetTile.ExpectedX)));
            var h = Math.Max(1, Math.Min(target.Rows, Math.Min(anchorTile.ExpectedY + anchorTile.Height, targetTile.ExpectedY + targetTile.Height) - Math.Max(anchorTile.ExpectedY, targetTile.ExpectedY)));
        ...
        private static RecoveryEdgeReport CreateRecoveryEdge(int anchorOrder, int targetOrder, string direction, string matcher, string reason, double[,] expected, double[,] measured, double[,] residual, MatchResult phase, double overlap, RecoveryEdgePurpose purpose, bool accepted)
        {
            var phaseScore = phase == null ? double.NaN : phase.RawScore;
            return new RecoveryEdgeReport { Purpose = purpose, Accepted = accepted, AnchorOrderIndex = anchorOrder, TargetOrderIndex = targetOrder, Direction = direction, Matcher = matcher, Reason = reason, TargetToAnchorTransform = measured, ExpectedTargetToAnchorTransform = expected, MeasuredTargetToAnchorTransform = measured, ResidualTransform = residual, ResidualDx = residual == null ? double.NaN : residual[0, 2], ResidualDy = residual == null ? double.NaN : residual[1, 2], PairMeasuredRotationDeg = 0, PhaseScore = phaseScore, EccCorrelation = double.NaN, OverlapRatio = overlap, Quality = Math.Max(0, phaseScore) * Math.Max(0, overlap) };
        }
        ...
        { 
            ...
            { 
                MinOverlapRatio = d.Geometry.MinOverlapRatio, MaxAbsRotationDeg = d.Geometry.MaxAbsRotationDeg,
                AllowNccOnlyAcceptance = d.Policy.AllowCoarseOnlyAcceptance, NccMinScore = d.Ncc.MinScore,
                EccMinCorrelation = d.Ecc.MinCorrelation, MaxTranslationPixels = d.Geometry.MaxTranslationPixels,
                MinScale = d.Geometry.MinScale, MaxScale = d.Geometry.MaxScale,
                AllowEccFromExpectedWhenNccFails = d.Policy.AllowRefinementFromExpectedWhenCoarseFails,
                PyramidLevels = d.Ecc.PyramidLevels, EccIterations = d.Ecc.MaxIterations, EccEpsilon = d.Ecc.Epsilon,
                Preprocessing = new PreprocessingOptions { Contrast = d.Preprocessing.Contrast }
            }; 
        }
        private static OpenCvSharp.Rect? ToNccRoi(SampleTileInfo tile)
        {
            if (!tile.NccModelRoiX.HasValue || !tile.NccModelRoiY.HasValue || !tile.NccModelRoiWidth.HasValue || !tile.NccModelRoiHeight.HasValue) return null;
            return new OpenCvSharp.Rect(tile.NccModelRoiX.Value, tile.NccModelRoiY.Value, tile.NccModelRoiWidth.Value, tile.NccModelRoiHeight.Value);
        }
        private static OpenCvSharp.Rect? ToShapeRoi(SampleTileInfo tile) { if(!tile.ShapeModelRoiX.HasValue||!tile.ShapeModelRoiY.HasValue||!tile.ShapeModelRoiWidth.HasValue||!tile.ShapeModelRoiHeight.HasValue)return null;return new OpenCvSharp.Rect(tile.ShapeModelRoiX.Value,tile.ShapeModelRoiY.Value,tile.ShapeModelRoiWidth.Value,tile.ShapeModelRoiHeight.Value); }
        ...
        private SampleTileContentMetrics GetSampleContent(AlignStitchConfig config, SampleTileInfo tile)
        {
            if (!config.RevalidateSampleContent && !string.IsNullOrWhiteSpace(tile.SampleContentClass))
            {
                ...
                if (Enum.TryParse(tile.SampleContentClass, out parsed)) return new SampleTileContentMetrics { ContentClass = parsed, MinGray = tile.SampleMinGray, MaxGray = tile.SampleMaxGray, MeanGray = tile.SampleMeanGray, StdDevGray = tile.SampleStdDevGray, NonZeroPixelRatio = tile.SampleNonZeroPixelRatio };
            }
#if DEBUG
...
#endif
            return new SampleTileContentAnalyzer().Analyze(_imageCache.GetMono8(tile.ExpectedPath), config.LowTextureStdDevThreshold);
        }

        private static TileWorkflowState WithContent(TileWorkflowState state, SampleTileContentMetrics metrics) { state.SampleContent = metrics; return state; }
        ...
        private static void ApplyStateReport(ProcessingReport report, TileWorkflowState state, SampleTileInfo tile)
        {
            ...
            if (item == null) 
            { 
                item = new ProcessingTileReport { OrderIndex = state.OrderIndex, Row = state.Row, Column = state.Column }; 
        ...
        {
            public RecoveryCandidate(TileWorkflowState anchor, string priority, bool traversalDirectionPreferred) { Anchor = anchor; Priority = priority; TraversalDirectionPreferred = traversalDirectionPreferred; }
