...
{
    // [Tony] [Change time: 2026-08-01] [Purpose: Centralize phase-only neighbor expected/measured/residual transform
    // semantics.]
    public static class NeighborTransformModel
    {
        ...
        public static Transform2D Residual(Transform2D expectedTargetToAnchor, Transform2D measuredTargetToAnchor)
        {
            if (expectedTargetToAnchor == null)
                throw new ArgumentNullException("expectedTargetToAnchor");
            if (measuredTargetToAnchor == null)
                throw new ArgumentNullException("measuredTargetToAnchor");
        ...
        {
            if (target.Column == anchor.Column + 1 && target.Row == anchor.Row)
                return "right";
            if (target.Column == anchor.Column - 1 && target.Row == anchor.Row)
                return "left";
            if (target.Row == anchor.Row + 1 && target.Column == anchor.Column)
                return "bottom";
            if (target.Row == anchor.Row - 1 && target.Column == anchor.Column)
                return "top";
            return null;
