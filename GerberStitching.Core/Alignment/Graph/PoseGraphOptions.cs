...
{
    // [Tony] [Change time: 2026-08-04] [Purpose: Configure the global pose-graph least squares optimizer that replaces
    // single-path neighbor propagation.]
    [TypeConverter(typeof(ExpandableObjectConverter))]
    public sealed class PoseGraphOptions
    {
        [Category("Pose graph")]
        [Description("Solves all tile poses jointly by robust least squares over neighbor edges. Supersedes " +
                     "single-path propagation and DirectPoseOutlierCorrector.")]
        public bool Enabled { get; set; } = true;

        [Category("Prior weights")]
        [Description("Prior weight pulling a SampleAlignment tile toward its direct pose, scaled by the direct " +
                     "selected common score.")]
        ...
        [Category("Prior weights")]
        [Description("Prior weight for exact-zero sample tiles. Non-zero so isolated blank tiles still land on the " +
                     "corrected grid.")]
        ...
        [Category("Edge gate")]
        [Description(
            "Loads neighbor edges that the legacy cycle-closure check rejected. Robust loss handles genuine outliers.")]
        public bool UseRejectedEdges { get; set; } = true;

        [Category("Edge gate")]
        [Description("Maximum translation deviation from the expected grid step before an edge is discarded as a " +
                     "period slip. Must stay below the capture overlap of 60 pixels.")]
        ...
        [Description("Emits a report warning for any used edge whose post-solve translation residual still exceeds " +
                     "this many pixels, so an unresolved seam gap is flagged instead of only visible in the stitched " +
                     "image. Zero disables the check.")]
