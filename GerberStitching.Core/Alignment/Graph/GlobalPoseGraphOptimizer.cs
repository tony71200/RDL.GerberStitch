...
{
    // [Codex] [Change time: 2026-08-04] [Purpose: Domain glue between AlignStitchWorkflowService state and the pose-graph solver: builds the problem, gates edges, applies the guarded solution back onto TileWorkflowState.]
    public sealed class GlobalPoseGraphOptimizer
    {
        public PoseGraphReport Optimize(AlignStitchConfig config,
            ...
            IList<RecoveryEdgeReport> edges)
        {
            ...
            for (var i = 0; i < ordered.Count; i++) nodeIndexByOrder[ordered[i].OrderIndex] = i;
            ...
            for (var i = 0; i < ordered.Count; i++)
            {
                ...
                if (hasPrior)
                {
                    ...
                    var a = priorMatrix[0, 0]; var b = priorMatrix[1, 0];
                    var k = Math.Sqrt(a * a + b * b);
                    node.Scale = k > 1e-9 && !double.IsNaN(k) ? k : 1.0;
                }
                nodes[i] = node;
                originalTx[i] = tx; originalTy[i] = ty; originalTheta[i] = theta;
            }

            var minPhase = config != null && config.NeighborAlignment != null && config.NeighborAlignment.Phase != null
                ? config.NeighborAlignment.Phase.MinResponse : 0.0;
            var minOverlap = config != null && config.NeighborAlignment != null && config.NeighborAlignment.Acceptance != null
                ? config.NeighborAlignment.Acceptance.MinOverlapRatio : 0.0;
            ...
            foreach (var e in fullEdges)
            {
                ...
                if (!dedup.TryGetValue(key, out existing) || e.PhaseScore > existing.PhaseScore) dedup[key] = e;
            }
            ...
            {
                var e = kv.Value;
                var entry = new PoseGraphEdgeEntry
                {
                    AnchorOrderIndex = e.AnchorOrderIndex,
                    TargetOrderIndex = e.TargetOrderIndex,
                    Direction = e.Direction,
                    PhaseScore = e.PhaseScore,
                    WasRejectedByLegacyClosure = !e.Accepted
                };
                string gateReason = null;
                int anchorIndex, targetIndex;
                if (!options.UseRejectedEdges && !e.Accepted) gateReason = "Not accepted by legacy pipeline and UseRejectedEdges=false.";
                else if (!nodeIndexByOrder.TryGetValue(e.AnchorOrderIndex, out anchorIndex) || !nodeIndexByOrder.TryGetValue(e.TargetOrderIndex, out targetIndex))
                    gateReason = "Anchor or target tile is excluded from the pose graph.";
                else if (e.MeasuredTargetToAnchorTransform == null || !Homography.IsFinite(e.MeasuredTargetToAnchorTransform))
                    gateReason = "Measured transform is not finite.";
                else if (e.PhaseScore < minPhase) gateReason = "PhaseScore " + e.PhaseScore.ToString("0.###") + " below Phase.MinResponse " + minPhase.ToString("0.###") + ".";
                else if (e.OverlapRatio < minOverlap) gateReason = "OverlapRatio " + e.OverlapRatio.ToString("0.###") + " below Acceptance.MinOverlapRatio " + minOverlap.ToString("0.###") + ".";
                else
                {
                    ...
                    if (residualNorm > options.MaxEdgeResidualPixels)
                        gateReason = "Residual vs expected grid " + residualNorm.ToString("0.###") + " px exceeds MaxEdgeResidualPixels " + options.MaxEdgeResidualPixels.ToString("0.###") + " (possible period slip).";
                    else if (Math.Abs(e.PairMeasuredRotationDeg) > options.MaxEdgeRotationDeg)
                        gateReason = "Rotation " + e.PairMeasuredRotationDeg.ToString("0.###") + " deg exceeds MaxEdgeRotationDeg " + options.MaxEdgeRotationDeg.ToString("0.###") + ".";
                }
                ...
                if (gateReason != null)
                {
                    ...
                    edgeGateReasons.Add("anchor=" + e.AnchorOrderIndex + ", target=" + e.TargetOrderIndex + ": " + gateReason);
                    continue;
                }
                ...
                var mTheta = Math.Atan2(e.MeasuredTargetToAnchorTransform[1, 0], e.MeasuredTargetToAnchorTransform[0, 0]);
                var graphEdge = new PoseGraphEdge
                {
                    ...
                    MTheta = mTheta,
                    BaseWeight = Math.Sqrt(Math.Max(e.PhaseScore, 1e-3)) * Math.Sqrt(Math.Max(e.OverlapRatio, 1e-3))
                };
            ...
            var union = new int[nodes.Length];
            ...
            for (var i = 0; i < nodes.Length; i++)
                if (edgeCountByNode[i] == 0) isolatedIndices.Add(i);
            report.IsolatedTiles = isolatedIndices.Select(i => nodes[i].OrderIndex).OrderBy(x => x).ToList();

            var problem = new PoseGraphProblem { Nodes = nodes, Edges = graphEdges.ToArray() };
            ...
            AssignPercentiles(solveStats.BeforeEdgeResiduals, out var beforeMedian, out var beforeP90, out var beforeMax);
            AssignPercentiles(solveStats.AfterEdgeResiduals, out var afterMedian, out var afterP90, out var afterMax);
            report.BeforeResidualMedian = beforeMedian; report.BeforeResidualP90 = beforeP90; report.BeforeResidualMax = beforeMax;
            report.AfterResidualMedian = afterMedian; report.AfterResidualP90 = afterP90; report.AfterResidualMax = afterMax;
            ...
            report.PoseDeltaMedian = deltaMedian; report.PoseDeltaP90 = deltaP90; report.PoseDeltaMax = deltaMax;
            ...
            if (options.MaxPoseCorrectionPixels > 0)
            {
                for (var i = 0; i < nodes.Length; i++)
                {
                    if (deltas[i] <= options.MaxPoseCorrectionPixels) continue;
                    applied = false;
                    report.Warnings.Add("Pose graph guard: OrderIndex " + nodes[i].OrderIndex + " would move " +
                        deltas[i].ToString("0.###") + " px, exceeding MaxPoseCorrectionPixels " + options.MaxPoseCorrectionPixels.ToString("0.###") + ". Result discarded; legacy poses kept.");
                }
            }
            ...
            {
                if (edgeDiagnostics[e].GateReason != null) continue;
                var matchIndex = graphEdges.FindIndex(g => g.AnchorIndex == nodeIndexByOrder[edgeDiagnostics[e].AnchorOrderIndex] && g.TargetIndex == nodeIndexByOrder[edgeDiagnostics[e].TargetOrderIndex]);
                ...
                if (options.EdgeResidualWarningPixels > 0 && edgeDiagnostics[e].ResidualAfter > options.EdgeResidualWarningPixels)
                {
                    report.Warnings.Add("Pose graph: edge OrderIndex " + edgeDiagnostics[e].AnchorOrderIndex + " <-> " + edgeDiagnostics[e].TargetOrderIndex +
                        " still has a " + edgeDiagnostics[e].ResidualAfter.ToString("0.###") + " px residual after solving (Huber weight " +
                        edgeDiagnostics[e].FinalHuberWeight.ToString("0.###") + "), exceeding EdgeResidualWarningPixels " +
                        options.EdgeResidualWarningPixels.ToString("0.###") + ". The seam at this boundary may not be fully continuous.");
                }
            }
            ...
            if (applied)
            {
                for (var i = 0; i < nodes.Length; i++)
                {
                    ...
                    var cos = Math.Cos(node.Theta); var sin = Math.Sin(node.Theta);
                    ...
                    if (afterSource != beforeSource)
                    {
                        ...
                        state.Reason = "Pose graph: jointly solved from " + edgeCountByNode[i] + " neighbor edge(s); global similarity scale=" +
                            report.GlobalScale.ToString("0.#####") + ", rotation=" + report.GlobalRotationDeg.ToString("0.###") + " deg.";
                    ...
                    tileEntries.Add(new PoseGraphTileEntry
                    {
                        ...
                        BeforeTx = originalTx[i], BeforeTy = originalTy[i], BeforeRotationDeg = originalTheta[i] * 180.0 / Math.PI,
                        AfterTx = node.Tx, AfterTy = node.Ty, AfterRotationDeg = node.Theta * 180.0 / Math.PI,
                        DeltaPixels = deltas[i], DeltaRotationDeg = (node.Theta - originalTheta[i]) * 180.0 / Math.PI,
                        Lambda = node.Lambda, EdgeCount = edgeCountByNode[i]
                    });
                }
            }
        ...
        {
            if (before == PoseSource.Failed) return hasEdges ? PoseSource.NeighborAlignment : PoseSource.Failed;
            return before;
        }
        ...
        private static double LambdaFor(TileWorkflowState state, PoseGraphOptions options)
        {
            switch (state.Source)
            {
                case PoseSource.SampleAlignment:
                    var score = state.Alignment != null && !double.IsNaN(state.Alignment.SelectedCommonScore) ? state.Alignment.SelectedCommonScore : 1.0;
        ...
        private static long KeyFor(int a, int b) { var lo = Math.Min(a, b); var hi = Math.Max(a, b); return ((long)lo << 32) | (uint)hi; }

        private static double ResidualNormFrom(RecoveryEdgeReport e)
        {
            if (e.ExpectedTargetToAnchorTransform == null || e.MeasuredTargetToAnchorTransform == null) return 0.0;
        ...
        {
            var a = nodes[edge.AnchorIndex]; var b = nodes[edge.TargetIndex];
            var ca = Math.Cos(a.Theta); var sa = Math.Sin(a.Theta);
            var dtx = b.Tx - a.Tx; var dty = b.Ty - a.Ty;
        ...
        {
            if (values == null || values.Length == 0) { median = 0; p90 = 0; max = 0; return; }
        ...
        private static int Find(int[] union, int i) { while (union[i] != i) { union[i] = union[union[i]]; i = union[i]; } return i; }
        private static void Union(int[] union, int a, int b) { var ra = Find(union, a); var rb = Find(union, b); if (ra != rb) union[ra] = rb; }
    }
}
