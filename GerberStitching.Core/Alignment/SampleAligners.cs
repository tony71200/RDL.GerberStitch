...
namespace GerberViewer.Stitching.Alignment
{
    public sealed class HalconNccSampleAligner : ISampleAligner
    {
        ...
        public SampleAlignmentResult Align(SampleAlignmentContext context)
        {
            ...
            if (context.Options == null)
                context.Options = new SampleAlignmentOptions();
            ...
            {
                context.Options.AllowNccOnlyAcceptance = original;
            }
        }
        ...
        private sealed class NccOnlyFactory : IMatcherFactory
        {
            ...
            {
                return kind == MatcherKind.HalconNcc ? _factory.Create(kind)
                                                     : new RejectingMatcher("EccDisabledForNccOnlyAligner");
            }
        }
    }
    ...
    public sealed class PyramidEccSampleAligner : ISampleAligner
    {
        ...
        private sealed class EccOnlyFactory : IMatcherFactory
        {
            ...
            public IMatcher Create(MatcherKind kind)
            {
                return kind == MatcherKind.PyramidEcc ? _factory.Create(kind)
                                                      : new RejectingMatcher("NccDisabledForEccOnlyAligner");
            }
        }
    }
    ...
    public sealed class NccThenPyramidEccSampleAligner : ISampleAligner
    {
        ...
        public NccThenPyramidEccSampleAligner(MatcherPipeline pipeline)
            : this(pipeline, new ModalityAwarePreprocessor())
        {
        }
        ...
        public SampleAlignmentResult Align(SampleAlignmentContext context)
        {
            ...
            var candidates =
                _preprocessor.PreprocessCandidates(context.SampleImage, context.CapturedImage, options.Preprocessing);
            SampleAlignmentResult bestRejected = null;
            try
            {
                foreach (var candidate in candidates)
                {
                    var request = new MatchRequest { ReferenceImage = candidate.Sample,
                                                     MovingImage = candidate.Captured,
                                                     InitialMovingToReferenceTransform =
                                                         ToTransform(context.InitialCapturedToSampleTransform ??
                                                                     context.ExpectedCapturedToSampleTransform),
                    ...
                    var pipelineResult =
                        _pipeline.MatchDirect(request, options.AllowNccOnlyAcceptance,
                                              options.AllowEccFromExpectedWhenNccFails, context.CancellationToken);
            ...
            return bestRejected ??
                   SampleAlignmentResult.Rejected(SampleAlignmentMethod.NccThenPyramidEcc, "DirectRejected");
        }
        ...
        private static SampleAlignmentResult ToAlignmentResult(MatcherPipelineResult pipelineResult, string variant,
                                                               SampleAlignmentContext context)
        {
            if (pipelineResult == null || !pipelineResult.Success || pipelineResult.SelectedCandidate == null ||
                !pipelineResult.SelectedCandidate.Success)
            {
                var rejected = SampleAlignmentResult.Rejected(SampleAlignmentMethod.NccThenPyramidEcc,
                                                              pipelineResult == null ? "DirectRejected"
                                                                                     : pipelineResult.FailureReason);
            ...
            result.Warning =
                pipelineResult.UsedCoarseFallback
                    ? "NccOnlyAcceptedAfterEccFailure"
                    : (pipelineResult.UsedRefinementFromExpected ? "EccAcceptedFromExpectedAfterNccFailure" : null);
            ...
            if (pipelineResult.CoarseCandidate != null && !pipelineResult.CoarseCandidate.Success)
                result.NccFailureReason =
                    pipelineResult.CoarseCandidate.FailureReason + ": " + pipelineResult.CoarseCandidate.FailureMessage;
            if (pipelineResult.RefinementCandidate != null && !pipelineResult.RefinementCandidate.Success)
                result.EccFailureReason = pipelineResult.RefinementCandidate.FailureReason + ": " +
                                          pipelineResult.RefinementCandidate.FailureMessage;
            return result;
        }

        private static void FillScores(SampleAlignmentResult result, MatcherPipelineResult pipelineResult)
        {
            result.NccScore = pipelineResult != null && pipelineResult.CoarseCandidate != null
                                  ? pipelineResult.CoarseCandidate.RawScore
                                  : double.NaN;
            result.EccCorrelation = pipelineResult != null && pipelineResult.RefinementCandidate != null
                                        ? pipelineResult.RefinementCandidate.RawScore
                                        : double.NaN;
        }

        private static SampleAlignmentResult BuildResult(SampleAlignmentMethod method, double[,] h, string variant,
                                                         SampleAlignmentContext ctx)
        {
            var r = new SampleAlignmentResult { Method = method,
                                                Success = true,
                                                CapturedToSampleTransform = h,
                                                PreprocessingVariant = variant,
                                                TranslationX = h[0, 2],
                                                TranslationY = h[1, 2],
                                                RotationDeg = Math.Atan2(h[1, 0], h[0, 0]) * 180 / Math.PI,
                                                Scale = Math.Sqrt(h[0, 0] * h[0, 0] + h[1, 0] * h[1, 0]) };
        ...
        {
            var sx = c.SampleImage.Width;
            var sy = c.SampleImage.Height;
            var cx = c.CapturedImage.Width;
            var cy = c.CapturedImage.Height;
        ...
        {
            return new MatcherOptions { NccMinScore = options.NccMinScore,
        ...
        private static SampleAlignmentResult PreferMoreInformative(SampleAlignmentResult current,
                                                                   SampleAlignmentResult candidate)
        {
            if (current == null)
                return candidate;
            if (!double.IsNaN(candidate.NccScore) && double.IsNaN(current.NccScore))
                return candidate;
            if (!double.IsNaN(candidate.EccCorrelation) && double.IsNaN(current.EccCorrelation))
                return candidate;
            return current;
        }
        ...
        private static void Validate(SampleAlignmentContext c)
        {
            if (c == null)
                throw new ArgumentNullException("context");
            if (c.SampleImage == null)
                throw new ArgumentException("SampleImage is required");
            if (c.CapturedImage == null)
                throw new ArgumentException("CapturedImage is required");
        }
    }
    ...
    internal sealed class RejectingMatcher : IMatcher
    {
        ...
        public RejectingMatcher(string reason)
        {
            _reason = reason;
        }
        ...
        {
            get {
                return "RejectingMatcher";
            }
        }
        public MatchResult Match(MatchRequest request, System.Threading.CancellationToken cancellationToken)
        {
