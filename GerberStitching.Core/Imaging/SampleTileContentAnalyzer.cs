...
{
    public enum SampleTileContentClass
    {
        Matchable,
        ExactZero,
        UniformNonZero,
    ...
    // [Tony] [Change time: 2026-07-27] [Purpose: Distinguish exact-zero samples from uniform nonzero and merely
    // low-texture samples before matching.]
    public sealed class SampleTileContentAnalyzer
    {
        public SampleTileContentMetrics Analyze(Mat source, double lowTextureStdDevThreshold)
        {
            ...
            {
                if (gray.Depth() != MatType.CV_8U && gray.Depth() != MatType.CV_16U)
                    return Unsupported();
                double min, max;
                ...
                var contentClass =
                    min == 0.0 && max == 0.0 && nonZero == 0
                        ? SampleTileContentClass.ExactZero
                        : (stddev.Val0 == 0.0 ? SampleTileContentClass.UniformNonZero
                                              : ((stddev.Val0 < lowTextureStdDevThreshold || ratio < 0.001)
                                                     ? SampleTileContentClass.LowTexture
                                                     : SampleTileContentClass.Matchable));
                return new SampleTileContentMetrics { ContentClass = contentClass,
                                                      MinGray = min,
                                                      MaxGray = max,
                                                      MeanGray = mean.Val0,
                                                      StdDevGray = stddev.Val0,
                                                      NonZeroPixelRatio = ratio };
            }
        }
        ...
        private static SampleTileContentMetrics Unsupported()
        {
            return new SampleTileContentMetrics { ContentClass = SampleTileContentClass.Unsupported };
        }
    }
}
