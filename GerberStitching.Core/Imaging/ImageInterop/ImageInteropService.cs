    ...
    public sealed class ImageInteropService : IImageInteropService
    {
        ...
        public HObject ToHObjectCopy(Mat source, InteropPixelFormat targetFormat)
        {
            ...
            try
            {
                ...
                {
                    if (targetFormat == InteropPixelFormat.Mono8) return AllocateMonoImageCopy(prepared, "byte");
                    if (targetFormat == InteropPixelFormat.Mono16) return AllocateMonoImageCopy(prepared, "uint2");
                    if (targetFormat == InteropPixelFormat.Bgr8) return AllocateColorImageCopy(prepared, "bgr");
                    if (targetFormat == InteropPixelFormat.Rgb8) return AllocateColorImageCopy(prepared, "rgb");
            ...
            catch (HOperatorException ex)
            {
                ...
                var guidance = expired
                    ? "HALCON license feature has expired (error 2042). Renew/select a valid HALCON 25.05 license or configure the Direct pipeline to use the OpenCV refinement fallback; pixel conversion cannot repair an expired license."
        ...
        {
            // [Codex] [Change time: 2026-08-02] [Purpose: Avoid gen_image1 feature-expiry failures and give HALCON ownership of the copied pixel buffer.]
            using (var contiguous = source.Clone())
            {
                ...
                HObject image = null;
                HTuple pointer = null; HTuple type = null; HTuple width = null; HTuple height = null;
                try
                {
        ...
        private static HObject AllocateColorImageCopy(Mat source, string colorFormat)
        {
            using (var contiguous = source.Clone())
            {
                ...
                {
                    var o = i * 3;
                    red[i] = bytes[o + (sourceIsBgr ? 2 : 0)]; green[i] = bytes[o + 1]; blue[i] = bytes[o + (sourceIsBgr ? 0 : 2)];
                }
                HObject r = null; HObject g = null; HObject b = null; HObject rgb = null;
                try
                {
                    ...
                    var result = rgb; rgb = null; return result;
                }
                finally
                {
                    if (rgb != null) rgb.Dispose(); if (b != null) b.Dispose(); if (g != null) g.Dispose(); if (r != null) r.Dispose();
                }
            }
        }
        ...
        {
            HObject image = null; HTuple pointer = null; HTuple type = null; HTuple width = null; HTuple height = null;
            try
            {
            ...
            {
                if (image != null) image.Dispose(); DisposeTuple(pointer); DisposeTuple(type); DisposeTuple(width); DisposeTuple(height);
            }
        }

        private static void ValidateAllocatedImage(HTuple type, HTuple width, HTuple height, string expectedType, int expectedWidth, int expectedHeight)
        {
            if (type == null || width == null || height == null || !string.Equals(type.S, expectedType, StringComparison.OrdinalIgnoreCase) || width.I != expectedWidth || height.I != expectedHeight)
                throw new InvalidOperationException("HALCON-owned image allocation does not match the requested pixel contract.");
        }
        ...
        private static int GetHalconErrorCode(HOperatorException exception)
        {
            ...
            if (propertyValue is int) return (int)propertyValue;
