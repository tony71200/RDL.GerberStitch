...
namespace GerberViewer.Stitching.Imaging
{
    ...
    public sealed class SampleCropResult
    {
    ...
    {
        public Task<SampleCropResult> GenerateAsync(PreparedSampleRun preparedRun, string outputRoot,
        ...
        // Allow Tab 2 to choose the sample folder name instead of always using the generated run identifier.
        public Task<SampleCropResult> GenerateAsync(PreparedSampleRun preparedRun, string outputRoot, string folderName,
                                                    CancellationToken cancellationToken,
                                                    IProgress<SampleCropProgress> progress)
        {
            return Task.Run(() => Generate(preparedRun, outputRoot, folderName, cancellationToken, progress),
                            cancellationToken);
        }

        private SampleCropResult Generate(PreparedSampleRun run, string outputRoot, string folderName,
                                          CancellationToken token, IProgress<SampleCropProgress> progress)
        {
            if (run == null)
            ...
            var final =
                Path.Combine(root, string.IsNullOrWhiteSpace(folderName) ? "GerberSample_" + runId : folderName);
            ...
            try
            {
                ...
                // Generate .ncm/.shm files only in Pregenerate mode. OnTheFly preserves the previous behavior and
                // creates models in memory.
                ...
                foreach (var tile in run.TilesByOrder)
                {
                    ...
                    progress?.Report(new SampleCropProgress { Completed = Math.Max(0, tile.OrderIndex), Total = total,
                                                              OrderIndex = tile.OrderIndex,
                                                              State = SampleTileState.Processing,
                                                              Message = "Cropping " + tile.OrderIndex });
                    var fileName =
                        FormatName(run.ConfigSnapshot.TileNamePattern, tile.Row, tile.Column, tile.OrderIndex) +
                        ExtensionFor(run.ConfigSnapshot.OutputFormat);
                    ...
                    using (var cropped = Crop(run.ProcessedImage, tile))
                    {
                        if (pregenerateModels)
                        {
                            ...
                            if (metadata.Metrics.ContentClass == SampleTileContentClass.Matchable)
                            {
                                ...
                                metadata.HasModel = true;
                                WriteShapeModel(cropped, shapeModelPath, metadata.X, metadata.Y, metadata.Width,
                                                metadata.Height);
                                metadata.HasShapeModel = true;
                            }
                    ...
                    if (pregenerateModels)
                    {
                        if (nccMetadata[tile.OrderIndex].HasModel)
                            VerifyNccModelReadable(modelPath);
                        if (nccMetadata[tile.OrderIndex].HasShapeModel)
                            VerifyShapeModelReadable(shapeModelPath);
                    }
                    progress?.Report(new SampleCropProgress { Completed = tile.OrderIndex + 1, Total = total,
                                                              OrderIndex = tile.OrderIndex,
                ...
                var finalValidation =
                    SampleManifestValidator.Validate(SampleManifestSerializer.Read(manifestPath), true);
                if (!finalValidation.IsValid)
                    throw new InvalidOperationException("Published manifest validation failed: " +
                                                        string.Join(Environment.NewLine, finalValidation.Errors));
        ...
        {
            HObject part = null;
            HOperatorSet.CropRectangle1(source, out part, tile.Rectangle.Top, tile.Rectangle.Left,
                                        tile.Rectangle.Bottom - 1, tile.Rectangle.Right - 1);
            return part;
        }
        ...
        private static void VerifyImageReadable(string path)
        {
            ...
            try
            {
                HOperatorSet.ReadImage(out read, path);
            }
            finally
            {
                if (read != null && read.IsInitialized())
                    read.Dispose();
            }
        }
        ...
        private static void ValidateTileFilesInTemp(PreparedSampleRun run, string tempRoot)
        {
            foreach (var tile in run.TilesByOrder)
            {
                var fileName = FormatName(run.ConfigSnapshot.TileNamePattern, tile.Row, tile.Column, tile.OrderIndex) +
                               ExtensionFor(run.ConfigSnapshot.OutputFormat);
                ...
                // var //modelPath = Path.Combine(tempRoot, "tiles", Path.GetFileNameWithoutExtension(fileName) +
                // ".ncm");
                //  Exact-zero samples intentionally have no matcher model; manifest validation enforces model presence
                //  for other classes.
            }
        }
            ...
            System.Collections.Generic.IDictionary<int, NccModelMetadata> nccMetadata)
        {
            return new SampleManifest {
                ...
                SourceRasterPath = run.ConfigSnapshot.SourceRasterPath, SourceWidth = run.SourceWidth,
                ...
                // Write the model-generation mode to the manifest so the Worker knows whether model files are included.
                ...
                Tiles =
                    run.TilesByOrder
                        .Select(
                            ...
                            {
                                var baseName =
                                    FormatName(run.ConfigSnapshot.TileNamePattern, t.Row, t.Column, t.OrderIndex);
                                // OnTheFly has no model metadata, so lookup must be null-safe rather than indexing
                                // directly.
                                ...
                                return new SampleTileInfo {
                                    ...
                                    ExpectedPath = Path.Combine(
                                        finalRoot, "tiles", baseName + ExtensionFor(run.ConfigSnapshot.OutputFormat)),
                                    NccModelPath =
                                        hasModel ? Path.Combine(finalRoot, "tiles", baseName + ".ncm") : null,
                                    NccModelRoiX = hasModel
                                    ? (int ?) metadata.X: null,
                                       NccModelRoiY = hasModel
                                       ? (int ?) metadata.Y: null,
                                          NccModelRoiWidth = hasModel
                                          ? (int ?) metadata.Width: null,
                                             NccModelRoiHeight = hasModel
                                             ? (int ?) metadata.Height: null,
                                                NccReferenceOriginRow = hasModel
                                                ? (double ?) 0.0: null,
                                                   NccReferenceOriginColumn = hasModel
                                                   ? (double ?) 0.0: null, NccModelSchemaVersion = hasModel ? 1 : 0,
                                                      ShapeModelPath = hasShapeModel ? Path.Combine(finalRoot, "tiles",
                                                                                                    baseName + ".shm")
                                                                                     : null,
                                                      ShapeModelRoiX = hasShapeModel
                                                      ? (int ?) metadata.X: null,
                                                         ShapeModelRoiY = hasShapeModel
                                                         ? (int ?) metadata.Y: null,
                                                            ShapeModelRoiWidth = hasShapeModel
                                                            ? (int ?) metadata.Width: null,
                                                               ShapeModelRoiHeight = hasShapeModel
                                                               ? (int ?) metadata.Height: null,
                                                                  ShapeReferenceOriginRow = hasShapeModel
                                                                  ? (double ?) 0: null,
                                                                     ShapeReferenceOriginColumn = hasShapeModel
                                                                     ? (double ?) 0: null,
                                                                        ShapeModelSchemaVersion = hasShapeModel ? 1 : 0,
                                                                        SampleContentClass =
                                                                            metadata == null
                                                                                ? null
                                                                                : metadata.Metrics.ContentClass
                                                                                      .ToString(),
                                                                        SampleMinGray = metadata == null
                                                                                            ? 0d
                                                                                            : metadata.Metrics.MinGray,
                                                                        SampleMaxGray = metadata == null
                                                                                            ? 0d
                                                                                            : metadata.Metrics.MaxGray,
                                                                        SampleMeanGray =
                                                                            metadata == null
                                                                                ? 0d
                                                                                : metadata.Metrics.MeanGray,
                                                                        SampleStdDevGray =
                                                                            metadata == null
                                                                                ? 0d
                                                                                : metadata.Metrics.StdDevGray,
                                                                        SampleNonZeroPixelRatio =
                                                                            metadata == null
                                                                                ? 0d
                                                                                : metadata.Metrics.NonZeroPixelRatio,
        ...
        // [Tony] [Change time: 2026-07-27] [Purpose: Create a content ROI model with a verified origin representing
        // full-tile (0,0).]
        private static NccModelMetadata WriteNccModel(HObject cropped, string modelPath)
        {
            ...
            HTuple area = null, centerRow = null, centerColumn = null, modelId = null, verifiedRow = null,
                   verifiedColumn = null;
            ...
            try
            {
                ...
                HOperatorSet.CreateNccModel(roiImage, options.NccNumLevels, options.NccAngleStartRad,
                ...
                if (Math.Abs(verifiedRow[0].D - offsetRow) > 1e-9 ||
                    Math.Abs(verifiedColumn[0].D - offsetColumn) > 1e-9)
            ...
            finally
            {
                ...
                DisposeTuple(height);
                DisposeTuple(row1);
                DisposeTuple(column1);
                DisposeTuple(row2);
                DisposeTuple(column2);
                DisposeTuple(area);
        ...
        {
            HObject roi = null, domain = null;
            HTuple id = null, area = null, row = null, column = null, vr = null, vc = null;
            var o = new HalconShapeModelOptions();
            try
            {
                HOperatorSet.CropRectangle1(cropped, out roi, y, x, y + height - 1, x + width - 1);
                HOperatorSet.GetDomain(roi, out domain);
                HOperatorSet.AreaCenter(domain, out area, out row, out column);
                HOperatorSet.CreateShapeModel(roi, o.NumLevels, o.AngleStartRad, o.AngleExtentRad, o.AngleStepRad,
                                              o.Optimization, o.Metric, o.Contrast, o.MinContrast, out id);
                var or = -(y + row[0].D);
                var oc = -(x + column[0].D);
                HOperatorSet.SetShapeModelOrigin(id, or, oc);
                HOperatorSet.GetShapeModelOrigin(id, out vr, out vc);
                if (Math.Abs(vr[0].D - or) > 1e-9 || Math.Abs(vc[0].D - oc) > 1e-9)
                    throw new InvalidOperationException("HALCON did not retain the requested shape model origin.");
                HOperatorSet.WriteShapeModel(id, path);
            }
            finally
            {
                if (id != null)
        ...
        {
            using (var mat = new ImageInteropService().ToMatCopy(cropped)) return new NccModelMetadata {
                Metrics = new SampleTileContentAnalyzer().Analyze(mat, 2.0)
            };
        }

        private sealed class NccModelMetadata
        {
            public int X;
            public int Y;
            public int Width;
            ...
            public SampleTileContentMetrics Metrics;
        }
        ...
        private static void VerifyShapeModelReadable(string path)
        {
            HTuple id = null;
            try
            {
                HOperatorSet.ReadShapeModel(path, out id);
            }
            finally
            {
                if (id != null)
        ...
        private static int BitDepth(HObject image)
        {
            ...
            try
            {
                ...
                if (value == "byte")
                    return 8;
                if (value == "uint2" || value == "int2")
                    return 16;
                if (value == "int4" || value == "real")
                    return 32;
                return 0;
            }
            finally
        ...
        {
            if (string.IsNullOrWhiteSpace(temp) || !Directory.Exists(temp) || string.IsNullOrWhiteSpace(marker) ||
                !File.Exists(marker))
                return;
        ...
        private static string ExtensionFor(SampleOutputFormat format)
        {
            ...
            case SampleOutputFormat.Png:
                return ".png";
            case SampleOutputFormat.Bmp:
                return ".bmp";
            case SampleOutputFormat.Jpeg:
                return ".jpg";
            default:
                return ".png";
            }
