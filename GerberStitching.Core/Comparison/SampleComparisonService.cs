...
namespace GerberViewer.Stitching.Comparison
{
    ...
    public sealed class SampleComparisonService
    {
        ...
        public SampleComparisonResult Generate(SampleComparisonRequest request, CancellationToken cancellationToken)
        {
            ...
            if (string.IsNullOrWhiteSpace(request.StitchedImagePath))
                throw new ArgumentException("StitchedImagePath is required.", "request");
            if (string.IsNullOrWhiteSpace(request.OutputDirectory))
                throw new ArgumentException("OutputDirectory is required.", "request");
            ...
            {
                result.Warnings.Add("Authoritative comparison blocked: " + mapping.Reason +
                                    " Non-authoritative visual preview is disabled.");
                WriteMetadata(result, request, mapping, null);
                return result;
            }
            if (!mapping.IsAuthoritative)
                result.Warnings.Add("Authoritative comparison blocked: " + mapping.Reason +
                                    " Generated products are non-authoritative visual previews only.");
            ...
            try
            {
                ...
                using (var sampleForComparison = PrepareComparable(
                    sample,
                    stitched,
                    mapping,
                    result.Warnings))
                using (var stitchedForComparison = PrepareComparable(
                    stitched,
                    sampleForComparison,
                    mapping,
                    result.Warnings))
                using (var samplePreview = MakePreview(
                    sampleForComparison,
                    request.MaxPreviewMegapixels))
                using (var stitchedPreview = MakePreview(
                    stitchedForComparison,
                    request.MaxPreviewMegapixels))
                using (var overlay = AlphaOverlay(
                    samplePreview,
                    stitchedPreview,
                    request.Alpha))
                using (var difference = AbsoluteDifference(
                    samplePreview,
                    stitchedPreview))
                using (var edge = EdgeOverlay(
                    samplePreview,
                    stitchedPreview))
                {
                    cancellationToken.ThrowIfCancellationRequested();
            ...
            {
                return GenerateHalconPreviewOnly(request, mapping, result,
                                                 "OpenCV comparison skipped for oversized image: " + ex.Message,
                                                 cancellationToken);
            }
            catch (ArgumentException ex)
            {
                return GenerateHalconPreviewOnly(request, mapping, result,
                                                 "Bitmap/OpenCV comparison skipped for oversized image: " + ex.Message,
                                                 cancellationToken);
            }
        }
        ...
        private static ComparisonMapping ResolveMapping(SampleManifest manifest, List<string> warnings)
        {
            if (!string.IsNullOrWhiteSpace(manifest.ProcessedSamplePath))
                return new ComparisonMapping(true, manifest.ProcessedSamplePath, null,
                                             "ProcessedSamplePath supplied by manifest.");
            if (manifest.SourceToProcessedTransform != null)
            {
                ...
                if (IsFiniteAffine(matrix))
                    return new ComparisonMapping(true, manifest.SourceRasterPath, matrix,
                                                 "SourceToProcessedTransform supplied by manifest.");
                warnings.Add("SourceToProcessedTransform is present but invalid/non-finite.");
            }
            if (manifest.ManifestVersion <= 1 && manifest.SourceWidth == manifest.ProcessedWidth &&
                manifest.SourceHeight == manifest.ProcessedHeight)
                return new ComparisonMapping(
                    true, manifest.SourceRasterPath, IdentityAffine(),
                    "Manifest v1 source dimensions equal processed dimensions; identity mapping is authoritative.");
            return new ComparisonMapping(false, FirstAvailablePath(manifest), null,
                                         "manifest lacks ProcessedSamplePath or a valid SourceToProcessedTransform " +
                                         "for ProcessedSampleGlobalPixels.");
        }

        private static string FirstAvailablePath(SampleManifest manifest)
        {
            if (!string.IsNullOrWhiteSpace(manifest.ProcessedSamplePath))
                return manifest.ProcessedSamplePath;
        ...
        private static Mat LoadSampleInProcessedSpace(SampleManifest manifest, ComparisonMapping mapping)
        {
            ...
            {
                if (!mapping.IsAuthoritative || !string.IsNullOrWhiteSpace(manifest.ProcessedSamplePath))
                    return source.Clone();
                if (mapping.SourceToProcessedTransform == null)
                    return source.Clone();
                using (var warp = ToWarp(mapping.SourceToProcessedTransform))
                {
                    var result = new Mat();
                    Cv2.WarpAffine(source, result, warp, new Size(manifest.ProcessedWidth, manifest.ProcessedHeight),
        ...
        {
            if (image.Rows == other.Rows && image.Cols == other.Cols)
                return image.Clone();
            if (mapping.IsAuthoritative)
                throw new InvalidOperationException("Authoritative comparison requires sample and stitched images to " +
                                                    "share ProcessedSampleGlobalPixels dimensions.");
        ...
        private static ComparisonMetrics ComputeMetrics(Mat sample, Mat stitched)
        {
            ...
            using (var absDiff = new Mat())
            {
                ...
                var f1 = edgePrecision + edgeRecall <= 0
                             ? 0
                             : 2.0 * edgePrecision * edgeRecall / (edgePrecision + edgeRecall);
        ...
        private static double NormalizedCrossCorrelation(Mat a, Mat b, Mat mask)
        {
            ...
            if (stdA.Val0 < 1e-6 || stdB.Val0 < 1e-6)
                return 0;
            using (var af = new Mat()) using (var bf = new Mat()) using (var da = new Mat()) using (
                var db = new Mat()) using (var prod = new Mat()) using (var maskFloat =
                                                                            MaskToFloat(mask)) using (var maskedProd =
                                                                                                          new Mat())
            {
        ...
        private static MetricDistribution DistanceStats(Mat referenceEdges, Mat stitchedEdges)
        {
            using (var inverted = new Mat()) using (var distance = new Mat())
            {
                ...
                using (var maskFloat = MaskToFloat(stitchedEdges)) using (var maskedDistance = new Mat())
                {
                    Cv2.Multiply(distance, maskFloat, maskedDistance);
                    return new MetricDistribution { Mean = Cv2.Sum(maskedDistance).Val0 / count,
                                                    P95 = MaskedPercentile(distance, stitchedEdges, 0.95),
                                                    Max = MaskedMax(distance, stitchedEdges) };
                }
            }
        }
        ...
        private static MetricDistribution AbsoluteDifferenceStats(Mat absDiff, Mat overlapMask)
        {
            ...
            if (count == 0)
                return new MetricDistribution();
            return new MetricDistribution { Mean = Cv2.Mean(absDiff, overlapMask).Val0,
                                            P95 = MaskedPercentile(absDiff, overlapMask, 0.95),
                                            Max = MaskedMax(absDiff, overlapMask) };
        }
        ...
        private static double MaskedPercentile(Mat values, Mat mask, double percentile)
        {
            ...
            for (int i = 0; i < 16; i++)
            {
                ...
                using (var threshold = new Mat()) using (var above = new Mat())
                {
        ...
        {
            using (var sampleGray = ToGray(sample)) using (var stitchedGray = ToGray(stitched)) using (
                var sampleEdges = Edges(sampleGray)) using (var stitchedEdges = Edges(stitchedGray))
            {
        ...
        private static Mat MakePreview(Mat source, double maxPreviewMegapixels)
        {
            ...
            if (scale >= 0.999)
                ...
                Cv2.Resize(source, result,
                           new Size(Math.Max(1, (int)(source.Cols * scale)), Math.Max(1, (int)(source.Rows * scale))),
        ...
        private static void SavePng(Mat image, string path)
        {
            if (!Cv2.ImWrite(path, image))
                throw new IOException("Failed to write comparison product: " + path);
        }

        private static SampleComparisonResult GenerateHalconPreviewOnly(SampleComparisonRequest request,
                                                                        ComparisonMapping mapping,
                                                                        SampleComparisonResult result, string reason,
                                                                        CancellationToken cancellationToken)
        {
            ...
            result.Warnings.Add("Generated HALCON preview-only images because the source is too large for OpenCV " +
                                "full-image comparison.");
            result.SamplePreviewPath = Path.Combine(request.OutputDirectory, "sample_reference_preview.png");
            result.StitchedPreviewPath = Path.Combine(request.OutputDirectory, "stitched_preview.png");
            WriteHalconPreview(mapping.SamplePath, result.SamplePreviewPath, request.MaxPreviewMegapixels,
                               cancellationToken);
            WriteHalconPreview(request.StitchedImagePath, result.StitchedPreviewPath, request.MaxPreviewMegapixels,
                               cancellationToken);
            WriteMetadata(result, request, mapping, result.Metrics);
            return result;
        }

        private static void WriteHalconPreview(string sourcePath, string outputPath, double maxPreviewMegapixels,
                                               CancellationToken cancellationToken)
        {
            if (string.IsNullOrWhiteSpace(sourcePath) || !File.Exists(sourcePath))
                throw new FileNotFoundException("HALCON preview image not found.", sourcePath);
            HObject image = null;
            ...
            try
            {
                ...
                if (scale >= 0.999)
                    HOperatorSet.CopyObj(image, out preview, 1, -1);
                else
                    HOperatorSet.ZoomImageSize(image, out preview, previewWidth, previewHeight, "constant");
        ...
        private static Mat ToWarp(double[,] h)
        {
            ...
            warp.Set<double>(0, 0, h[0, 0]);
            warp.Set<double>(0, 1, h[0, 1]);
            warp.Set<double>(0, 2, h[0, 2]);
            warp.Set<double>(1, 0, h[1, 0]);
            warp.Set<double>(1, 1, h[1, 1]);
            warp.Set<double>(1, 2, h[1, 2]);
            return warp;
        }

        private static double[,] MatrixFromJagged(double[][] source)
        {
            if (source == null || source.Length != 3 || source[0] == null || source[1] == null || source[2] == null ||
                source[0].Length != 3 || source[1].Length != 3 || source[2].Length != 3)
                return null;
            return new[,] { { source[0][0], source[0][1], source[0][2] },
        ...
        private static void WriteMetadata(SampleComparisonResult result, SampleComparisonRequest request,
                                          ComparisonMapping mapping, ComparisonMetrics metrics)
        {
            ...
            for (int i = 0; i < result.Warnings.Count; i++)
                sb.Append("    \"")
                    .Append(Escape(result.Warnings[i]))
                    .Append(i + 1 == result.Warnings.Count ? "\"\n" : "\",\n");
            sb.AppendLine("  ],");
        ...
        {
            public ComparisonMapping(bool isAuthoritative, string samplePath, double[,] sourceToProcessedTransform,
                                     string reason)
            {
